...
void AnabasisEngine::prepare (double sampleRate, int maxBlockSize, int numChannels)
{
    ...
    specInRing.reset();
    specOutRing.reset();
    // …and the GR history's PARTIAL entry with them (0.2.12, OQ-017 fix 1).
    // The ring itself is cleared by the wrapper and only when the (rate, block)
    // pair changed; this is the accumulator, and it is dropped on every
    // prepare regardless. A re-prepare is a break in the audio an entry
    // describes — a transport stop, a rate change, a rescan — so the samples
    // on the two sides of it must not end up in one entry. The cost is at most
    // `maxBlock - 1` samples of a partial entry, which is less than one entry
    // of the twenty-second window.
    histMinGain = 1.0f;
    histPeak    = 0.0f;
    histSamples = 0;
