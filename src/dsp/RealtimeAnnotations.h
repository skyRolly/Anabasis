...
//
//  WHAT IT IS. `ANABASIS_NONBLOCKING` expands to Clang's
...
//
//      bool process (juce::AudioBuffer<float>& b,
...
//  9.0.1 carries no annotations of its own (measured: zero occurrences of
//  `clang::nonblocking`, `clang::nonallocating` or `__rtsan` in the pinned
//  checkout), so annotating the call tree transitively produces warnings about
//  correct code wherever a JUCE call appears. ADR-0029 records the resulting
//  rule: the annotation marks ENTRY POINTS for the runtime tool, and
//  `-Wfunction-effects` stays off wherever JUCE is in the call graph.
//
//  It is NOT off everywhere. This engine has a genuinely JUCE-FREE leaf layer --
//  `CeilingClamp.h`, `ScopeBuffer.h`, `Latency.h` and `EngineParameters.h`, four
//  headers that include no JUCE module at all -- and over that layer the same
//  flag has nothing opaque to complain about. `tests/realtime_effects.cpp`
//  compiles exactly those under `-Werror=function-effects` in the `realtime`
//  job, which is the one thing a runtime tool structurally cannot do: it proves
//  the bodies effect-clean before any test executes them, including the branches
//  the suite never takes (ADR-0029).
//
...
// ============================================================================
    ...
    #define ANABASIS_NONBLOCKING [[clang::nonblocking]]
  #endif
#endif

#ifndef ANABASIS_NONBLOCKING
  #define ANABASIS_NONBLOCKING
#endif
