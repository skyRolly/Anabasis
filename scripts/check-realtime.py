#!/usr/bin/env python3
"""
Anabasis -- static realtime lint: forbidden operations inside audio-path bodies.
...
tool sees exactly the code the suite EXECUTES.  Measured on THIS tree (gcov,
GCC 13.3, -O0, `AnabasisTests` alone): the DSP suite covers **73.7 % of the
3,156 lines and 63.4 % of the 1,190 branches** in `src/dsp`.  So better than a
quarter of the lines and better than a third of the branches are never run, and
a `malloc` added to any of them is invisible to both runtime tiers and visible
here.  This lint is the cheap complement, not a third opinion on the same
...
====================================================
A file-wide token scan over `src` is unusable, and measurably so: all three
`setSize` calls in `AnabasisEngine.cpp` and every `.assign` in the modules sit
inside `prepare()`, where allocation is not merely allowed but REQUIRED (the
policy's own wording: "Buffer sizing must happen in prepare()").  A lint that
flags them is a lint that gets switched off.
...
# The policy binds `processBlock` / `AnabasisEngine::process` and every DSP
# module's `process`/`reset` -- and `AnabasisAudioProcessor::processBlock` lives
...
#
# THE NAME SET IS THIS PRODUCT'S, derived by enumerating the definitions in
# `src/` rather than copied from the sibling: `process`, `processBlock`,
# `processChunk`, `processFrame` and `processSample` are the five spellings this
# chain actually uses (the engine chunks a block larger than the prepared
# maximum, and the modules take a frame or a sample), while `pushBlock`,
# `finishBlock` and the `publish*` trio are the meter and §5.4 adaptive taps
# `AnabasisEngine::process` calls on its way through.
#
# The `reset` family is here because REALTIME_AUDIO_POLICY binds "every DSP
# module's `process`/`reset`": the engine calls `reset()` at the silent bottom of
# a §2.8 switch duck and on the NaN self-heal, and the meters' `resetWindow`,
# `resetEpoch`, `resetGeneration`, `resetMeterHolds` and `resetIntegrated` run
# from the same place. `REALTIME_SAFETY_AUDIT.md` audits them for that reason.
#
# MATCHING IS BY EXACT NAME, never by prefix, and that is what keeps the rest of
# `src/` out of the findings: `PresetManager::resetSlotFieldsToDefaults` and
# `MacroEngine::resetToMacro` are message-thread functions whose names begin with
# `reset`, and a `reset[A-Za-z]*` pattern would pull both into the scan and make
# the lint report on code the policy does not bind.
#
...
FORBIDDEN = [
    ...
    # (`AnabasisEngine.cpp`). A lint that catches `new T` but not
...
def _is_declarator_tail(clean: str, start: int, brace: int) -> bool:
    ...
    `ANABASIS_NONBLOCKING`, `override`/`final`, a trailing return type, a
    ...
    `AnabasisAudioProcessor` constructor. Two of those names, `isPresetExcluded`
...
def heads_a_definition(clean: str, at: int) -> bool:
    ...
    The discriminator is what comes BEFORE the name:
      ...
      * `::` -- a qualified definition (`void AnabasisEngine::process (`);
...
# On a no-argument definition (`void AnabasisEngine::updateDerived()`) the next
...
`AnabasisEngine::processChunk()` reaches the whole
    serial chain through helpers that are named for what they DO rather than for
    the policy's vocabulary -- the §2.8 duck's bottom-of-ramp rewire, the §5.4
    adaptive commit, the meter taps -- and every one of them is audio-thread
    code, in the same file as its caller, that a name-only scan never sees. A hand-maintained list of extra names would have the same
...
MUST_FIRE = [
    ...
    ("allocation in a module reset",
     "void MasteringComp::reset() noexcept { env.resize (n); }"),
    ("lock in a module reset",
     "void ClipSat::reset() noexcept { std::lock_guard<std::mutex> g (m); }"),
    # The `reset` family, which the engine calls at the silent bottom of a
    # transition duck and on the NaN self-heal -- audio-thread code with exactly
    # the same constraint as `process`, and where a `prepare()` line gets copied
    # to.
    ("allocation in LookaheadLimiter::resetWindow (the engine's reset path)",
     "void LookaheadLimiter::resetWindow() noexcept { wedgeValues.assign (n, 0.0f); }"),
    # `AnabasisAudioProcessor::processBlock` is the FIRST function the policy
    # names and it lives in `src/`, not `src/dsp/`.
    ("allocation in the wrapper processBlock",
     "void AnabasisAudioProcessor::processBlock (juce::AudioBuffer<float>& b, juce::MidiBuffer&) { v.push_back (1.0f); }"),
]
...
def self_test() -> int:
    ...
    # whole round is about. The tree's own 276-character `AnabasisAudioProcessor`
    # constructor is the second case.
    for label, region, want in [
        ("cv/ref qualifiers and noexcept", " const noexcept ", True),
        ("an effect macro", " noexcept ANABASIS_NONBLOCKING ", True),
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Anabasis static realtime lint.")
