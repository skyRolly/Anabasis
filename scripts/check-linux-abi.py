...
#  asserted rather than assumed. With a self-test, per TESTING_POLICY rule 5.
#
...
#  users.
#
#  MEASURED ON THIS REPOSITORY'S OWN ARTIFACTS (2026-08-22, GCC 13.3 on
#  `ubuntu-noble`, the shipped Release build): the VST3 and the Standalone BOTH
#  require GLIBC_2.38, GLIBCXX_3.4.31 and CXXABI_1.3.9. That is Ubuntu 23.10 /
#  Debian 13 for glibc and GCC 13 for libstdc++ -- so the VST3 does NOT load on
#  Ubuntu 22.04 LTS (glibc 2.35), a live LTS and a normal thing for a DAW user to
#  be running. Nobody decided that number. It is what `ubuntu-latest` moving from
#  22.04 to 24.04 did to the artifact, silently, in some earlier run, while
#  `COMPATIBILITY_MATRIX.md` said in as many words that no Linux OS floor "has
#  been decided or measured". This file is the measurement, and the gate that
#  keeps it true.
#
...
#  move one without the other -- a newer GCC's exception path pulls
#  `__cxa_call_terminate@CXXABI_1.3.15` (GCC 14) while its `GLIBCXX` reference
#  stays at 3.4.31 (GCC 13). Reading `GLIBCXX` alone, the gate would report a
#  GCC 13 floor and pass an artifact needing a GCC 14 runtime. A family the gate
#  does not name cannot raise the floor loudly, so it raises it silently:
#  exactly what this file exists to prevent.
#
#  EACH FLOOR IS THE MEASUREMENT, NOT A ROUND NUMBER ABOVE IT. `CXXABI_1.3.9` is
#  what this artifact actually requires today, and declaring the GCC 13 ceiling
#  (1.3.14) instead would leave five ABI versions of headroom in which a real
#  raise passes unnoticed -- which is the failure mode of a floor nobody
#  tightened. Sitting exactly on the measurement means any change that pulls one
#  symbol newer fails HERE, in the run that did it, with the family named. That
#  is the intended cost.
#
#  The sibling product's copy of this file declares `CXXABI_1.3.14` because that
#  is what ITS artifact needs; the numbers are per-product measurements and were
...
CXXABI_FLOOR = "1.3.9"      # what the shipped artifact actually requires (measured 2026-08-22)
...
"""
    ...
    expect("a binary within every floor is clean and complete",
           evaluate({"GLIBC": "2.35", "GLIBCXX": "3.4.30", "CXXABI": "1.3.9",
                     "GCC": "3.3.1"}),
           ([], []))
