...
| Field | Value |
|---|---|
| **Current Version** | **0.2.11** (`project(Anabasis VERSION 0.2.11)` in `CMakeLists.txt`), with a dated `## [0.2.11] — 2026-09-05` CHANGELOG entry — **the owner's second GR-history report: 0.2.8 had not fixed the part of the line that is being generated.** Reproduced frame by frame on the real processor and the real paint path (a scratch harness driving `GrHistoryView::tick`/`paintHistory` with a simulated host and frame clock, the 0.2.7 and pre-fix 0.2.8 painters ported beside it on the same frames), confirmed by the owner against the description, then fixed: the newest vertex was a live estimate — a minimum over a window that slid with every block, pinned to the edge while its bucket filled, released to drift once complete — so the last pitch of the line was revised on every block and re-shaped on every frame while the completed trace glided rigidly behind it; 0.2.8's claim to have halved that movement was not reproduced on the real limiter. A bucket is now drawn only once complete, at its final value, under the one rigid law every other vertex obeys (`Buckets::kLast`; `tipFirst` and the pinned-tip branch of `bucketX` are gone). The last plot column's blink — the lead-out ended one column short of the clip edge — is fixed in the same path. Validated on the harness against both predecessors: revisions of a drawn vertex 0 (pre-fix 24–36/s, up to 57 px), re-sloped segments 0, collapses 0, the rigid translation of the completed trace unchanged (one uniform step per frame, zero deviation on a steady host), the last column lit on every frame. Unchanged and filed as **OQ-017**: the scroll timing under bursty or mis-sized host delivery. Trail: `worklogs/2026-09-05-gr-history-tip.md`. Before that, **0.2.10** — **0.2.9's guard was too wide, and it guarded only half the door.** Review of that change found both faults. It tested `! std::isfinite`, which declines an INFINITY along with a NaN — but an infinity is not an unusable number, it is an out-of-range one, and every clamp on the path answers it with the endpoint (`jlimit` with 1 and 0, `snapToLegalValue` with the range ends, the `value` path with the same). A session asking for the rail therefore got `value` instead, and a preset asking for one left the control where it stood; the comment beside the code said "infinities already clamp correctly" while the code stopped them doing so. The predicate is now `std::isnan`, which is the value that actually needs it. SEPARATELY, 0.2.9 guarded the `raw` OVERLAY and left the `value` it falls back TO unguarded: with no usable `raw` and a NaN `value`, all 50 controls were still poisoned and the next save still wrote every one back. `adoptParamsTree` now drops an unreadable `value` before `replaceState`, which makes APVTS restore `getDenormalisedDefaultValue()` — the schema's own rule for a value that cannot be read, reached through JUCE's existing default path rather than a new mechanism. Two post-audit items ship with them: `AnabasisEngine::prepare` now rails the one route a sample rate has into an allocation (`delaySamples`, which sizes `wetRing` and `dryRing`; unreachable from a conforming host, and no positive rate changes), and `AnabasisBench` no longer throws on a `/proc/cpuinfo` whose `model name` value is empty. Trail: `worklogs/2026-09-03-review-followup.md`. |
