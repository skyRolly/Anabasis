...
| Field | Value |
|---|---|
| **Current Version** | **0.2.8** (`project(Anabasis VERSION 0.2.8)` in `CMakeLists.txt`), with a dated `## [0.2.8] — 2026-09-01` CHANGELOG entry — **the GR history scrolls continuously instead of standing still and lurching** (the owner's *"newly drawn portion … to the right of the yellow line is jittery"*; ADR-0023 item 6 amended in place, dated). The yellow line is the trace's own flat zero-reduction run, and the sloped part right of it was the only part able to show what `GrHistoryView::bucketX` had done since 0.1.2: move every vertex only when a decimation bucket completed, by a non-integer pitch (1.447 px at 48 kHz/512 on the Simple well). Modelled at 60 Hz: 48 % of frames motionless, the rest a 1.45 px lurch that re-rasterised the anti-aliased stroke at every step. `bucketX` now places each bucket by the newest ENTRY's sub-bucket position (`Buckets::fill`) and, between arrivals, by a head the frame clock smooths at the nominal entry rate and holds to `[head, head + 1]` (`smoothedHead`, `phaseOf`, `entryPeriod` — the same prepared pair `windowEntries` maps the window through) — `pitch / stride` per block, one uniform step per frame; the read window is rounded up to whole buckets so the oldest drawn vertex sits on or just beyond the left edge and the crossing segment is clipped rather than led in flat (the bucket-rate pop the review found at that edge); the newest vertex aggregates the trailing `stride` entries (`tipFirst`) instead of its bucket's partial range, so it no longer pops to a single block's value at every bucket start (0.99 → 0.55 dB modelled). Same model after the fix: 0 % motionless frames, per-frame σ 0.72 → 0.000 px on a steady host at every block size (the per-entry half alone reaches 0.24 at 512 and does nothing at stride 1 — 1024 at 44.1 kHz, 2048 at 48 kHz on the Simple well — which is what the smoothed head is for; its first draft, a since-arrival phase, was caught by review never engaging where the block rate exceeds the display rate). Bucket identity, every completed value, the fixed pitch, the right anchor, the zero region and the clear rule are unchanged; the reader contract and the audio-thread push are untouched, and no DSP algorithm, parameter, schema, threading-model or latency change. The geometry test walks the head one entry at a time across three buckets at every case and holds the per-entry motion, which the stepped form fails at every head; seventeen mutants run, each killing its own assertions; the pre-fix adversarial review's findings — the left-edge pop the first draft had left, the phase draft that never engaged, the tick→paint head mismatch — are acted on and recorded, and the one it raised that is a product trade (a lag buffer for hosts that render ahead of real time, at the cost of display latency) is recorded for the owner rather than taken. **A second review round then found three reader/publisher defects in that fix, all repaired here** (worklog §9): the frame drew its window from the head the TICK observed while deriving the ring's one-slot safety margin from that same stale head, so at a saturated window one block arriving in between put the oldest peek on the slot the audio thread was filling — the margin is measured against the LIVE index now (`readFloor`) and re-checked after the batch, with the ring's producer untouched; `shownHead` and `smoothHead` were plain scalars written by the message thread and read by the GL render thread on macOS/Windows, i.e. a data race — now relaxed atomics, `static_assert`ed lock-free, and safe as a PAIR by value rather than by snapshot (`frameFor`), which widens ADR-0027's Message → Painting row and therefore went to the Architecture Review Gate as [ADR-0038](architecture/design-decisions/ADR-0038-gr-history-display-scalars-cross-the-painting-boundary.md) — **Accepted 2026-09-02 on the owner's approval**, with ADR-0027 clause 4 amended in place (its boundary moves from "ONE scalar" to "scalars whose every stale/fresh pairing is a frame the writer was itself about to produce"; nothing else in that clause is reinterpreted); and the idle gate keyed on the entry COUNT, so a clear that refilled to the same count parked on new data and kept the old trace — gate and phase re-anchor now key on the ring's reset epoch. **A final review round then found the fourth and last one** (worklog §10): the guards that discard a raced frame read synchronised state, but what they validated did not — `GrHistoryBuffer` stored and read its payload with plain accesses, so an overtaken batch was a data race, undefined the moment it happened and beyond the reach of any after-the-fact discard. The stored fields are `std::atomic<float>` now, relaxed both ways, which makes the racing read DEFINED and leaves the guards' job unchanged; the audio-thread `push` compiles to an **instruction-identical** sequence (measured against the generated code, clang-22 `-O3`) and `is_always_lock_free` is `static_assert`ed, so the realtime contract is untouched. `ScopeBuffer` shares the idiom with ~0.26 s of reader headroom and is filed as **KI-015** rather than repaired, the 0.2.8 scope excluding it. **The last finding of the round** (worklog §11) was the reset phase: a paint runs on its own schedule and can land between `GrHistoryBuffer::reset()` and the tick that re-anchors, where reading the pre-clear `smoothHead` against the fresh head saturates `phaseOf`'s clamp to exactly 1 — the first frame of the new history drawn one entry-pitch out. A published display estimate now carries the IDENTITY of the state it describes: `publishedEpoch`, stored `release` last and loaded `acquire` first, is the boundary's only ordered access, and `frameFor` uses the phase only when it matches the epoch the frame is drawing. The count cannot stand in for it — once the refill passes the old count both `paintHead`'s fallback and an out-of-range test on `smoothHead - head` see a legitimate parked frame — and the test is mutation-verified against BOTH wrong answers, ignoring the epoch and inferring the reset from the head. ADR-0038 gains clause 7 and a dated amendment to clause 2 (`worklogs/2026-09-01-gr-history-scroll-jitter.md`). **A fourth round (worklog §12) found the last race**: the view mapped its window and scroll rate through `AudioProcessor::getSampleRate()`/`getBlockSize()` — plain members the host's reconfiguring thread writes (`setRateAndBufferSizeDetails`, before `prepareToPlay`) while the tick and the paint read them. The prepared pair is `GrHistoryBuffer` metadata now, stored by `clear` inside the reset epoch's window and read under the same bracket as the entries, with the clear-on-change gate moved into `GrHistoryBuffer::prepare`; and the bracket's close is `batchIntact` — an acquire FENCE plus a relaxed re-read, Boehm's seqlock reader, correct under the C++ model on weakly ordered targets where the acquire load it replaces was not (ADR-0011's time-series bullet carries the dated amendment, **accepted by the owner 2026-09-02 — that Architecture Review Gate item is cleared**; the audio thread's `push` is untouched). The same round returned the GCC 16 LTO lane to zero first-party warnings (six `-Wfloat-equal` exact comparisons in the new test, now tolerance checks) and moved that test's four 32 KB rings and one 76 KB processor to the heap for MSVC PREfast's C6262 (379,228 bytes of summed locals; under clang `-fno-inline` the function's frame fell 76,632 → 936 bytes). **A separate follow-up then closed KI-015** (`worklogs/2026-09-02-ki015-scopebuffer-payload.md`): the spectrum capture rings carried the same non-atomic payload the GR ring did until 0.2.8 — producer `memcpy`, reader plain subscripts — and the "12288 frames of headroom" that made it a follow-up rather than a blocker turned out not to be an invariant (a frame count rather than a time, 0.064 s at 192 kHz; `reset()` rewinds the head; and `maxBlock` is the host's unclamped `samplesPerBlock`, so one push reaches the reader's oldest slot at n ≥ 12289 and its whole window at n ≥ 16384, with no reader stall). The payload element is a `Sample` wrapper over one relaxed `std::atomic<float>` exposing only relaxed accessors, so the SEQ_CST `operator=` spelling is ill-formed; `SpectrumView` is untouched and the ring takes NO reader-side acquire fence, because its `reset()` writes one atomic and touches no sample. Measured rather than assumed, and the GR round's instruction-identity claim explicitly does not transfer: `pushBlock` goes from four `memcpy` call sites to zero calls and zero lock prefixes, costing +0.005 % of the block period at 48 kHz/512. **ADR-0011 gained a third dated 2026-09-02 amendment — raised at the Architecture Review Gate and held, because it supersedes an accepted sentence in the first amendment, which `AI_AGENT_POLICY.md` makes a Hard Stop regardless of the agent's own view of severity — and the owner ACCEPTED it on 2026-09-02, clearing that gate.** Two findings filed rather than fixed: **KI-016** (Anamorph carries the unrepaired shape, unrecorded there; accepted one-way drift under ADR-0009 item 8) and **KI-017** (`SpectrumView`/`CurveView` read JUCE's plain prepared rate from the painting thread — and, inside it, the record that JUCE 9.0.1 takes the MessageManager lock around `paintComponent`, which narrows ADR-0027's and ADR-0038's stated premise without disturbing either decision). Preceded by **0.2.7 (2026-08-30)** — **LLVM 23.1.0 is released; what apt ships for it is not, so the Clang pin HOLDS at 22 and the install learns to tell the difference** ([ADR-0037](architecture/design-decisions/ADR-0037-llvm-23-toolchain-upgrade.md)), amending ADR-0031 clauses 1 and 2. The directive asked for the newest stable released toolchain AND forbade release candidates, and those were different instructions: `apt.llvm.org`'s noble suite builds `clang-23` from `release/23.x` commit `55feb0a3b6b7`, after the `llvmorg-23.1.0-rc3` tag and before the release commit, still carrying `LLVM_VERSION_SUFFIX -rc3`. **Debian's packaging drops the suffix**, so `--version`, `__clang_version__` and the dpkg version all read a clean `23.1.0` — and that build cleared the warning gate, both suites, ASan+UBSan, the LTO lane and RTSan, because every check here was major-only. `setup-llvm-apt.sh` now asserts the package's upstream commit IS the release tag's commit (accepts 22.1.8's `ca7933e47d3a`, refuses 23.1.0's `55feb0a3b6b7`; the script exits 1 naming both). **23 was exercised end to end** on that same rc3-era build — zero first-party warnings, 301 + 873, ASan+UBSan 300 + 873, LTO 301 + 873 under `Ubuntu LLD 23.1.0`, RTSan 296 — which is evidence the tree is ready for 23 rather than a measurement of 23.1.0; re-run it against the release when the suite rebuilds, then the bump is one line. GCC audited and NOT moved (16.2 newest; the floating `gcc:16` already collected it). Preceded by **0.2.6, parity audit round 2** ([ADR-0036](architecture/design-decisions/ADR-0036-parity-audit-round-2.md)). The sibling's CI surface proved byte-identical to ADR-0034's baseline, so the fresh 200-row read yielded what that audit missed plus what 0.2.3–0.2.5 changed here. Nine adoptions, each verified before landing. The two measured fixes: **`MALLOC_PERTURB_` moved 255 → 1** — glibc fills FRESH allocations with the complement of the byte, so 255 wrote the benign 0x00 the variable exists to defeat (measured in both directions; the old NaN claim described freed-block reads) — and **`ANABASIS_BUILD_NUMBER` became a source-file property on `src/gui/PluginEditor.cpp`**, the 0.2.4 direct-mode finding landed with family precedent (2 of 103 compile commands now carry the define; suites 301 + 873, 0 failures under the new allocator byte). The structural adoptions: **macOS pluginval now validates the shipped `dist/` bytes** (packaging before validation, AU installed from dist — OQ-012's macOS half resolved; Windows half stays open), the **Rosetta x86_64 result gates the macOS uploads**, the **MSVC assert moved last** so a toolset move cannot destroy its own evidence, **ccache zero+print landed on 8/8 cached jobs**, `macos-intel` gained a **thin-slice assertion and randomise arms**, the GCC lane gates **three more GCC-only diagnostics at zero** (measured: 4 vendored hits, 0 first-party), the Windows PE guard covers the Magic read (+26), and both macOS jobs clean up the registry-installed AU. Kept-different re-affirmed item by item (zero-warning contract, no fuzz, `preflight`, no `TESTS_NO_FTZ`, CXXABI 1.3.9, `linux-lto-clang`, the instruments, the 0.2.5 dSYM gates, the 0.2.3 package set, GUI-inclusive Windows pluginval). Also recorded: the sibling's JUCE pin **re-converged** at 9.0.1 `e18f7f5` (DEPENDENCY_POLICY updated), and the sibling's own container lane and arm64 dSYMs are exposed to the defects Anabasis fixed at 0.2.3/0.2.5 — recorded only, Anamorph is read-only. No DSP algorithm, parameter, schema, threading-model or latency change; outside `.github/` and docs the only touched file is `CMakeLists.txt` (build-number scoping, output bytes identical). 0.2.5 (2026-08-22) was **the arm64 symbolication round**. Every macOS run emitted `warning: no debug symbols in executable (-arch arm64)` for VST3, AU and Standalone while the job reported a validated dSYM: on Apple Silicon the shipped bundle was not symbolicatable, and three checks said otherwise. **Cause:** a universal build links once per architecture and several targets link with LTO, so one build performed six links all naming the same `-Wl,-object_path_lto` destination; five were overwritten and the survivor was one target's x86_64 codegen. The `CMakeLists.txt` comment asserting that a directory makes ld64 generate unique names had it **backwards** — ld64 appends the fixed name `lto.o` to a directory (`ld/parsers/lto_file.cpp`), while a non-directory path is used verbatim, which is the branch that actually gives uniqueness. Each `(target, architecture)` now names its own file via `$<TARGET_PROPERTY:NAME>` plus `-Xarch_<arch>`; generated link lines show **6 distinct paths** for 3 targets × 2 architectures where 1 exists today. **Why nothing caught it:** a fat dSYM lists a UUID per slice whether or not that slice carries DWARF, so the UUID-set match passed; compile units were counted across the whole fat file, so 39 from x86_64 plus 0 from arm64 cleared `-eq 0`; and the retention assertion tested only `COUNT -eq 0`, reading 1 where its own comment expects one per (target × slice). All three now work per slice — `dwarfdump --arch <A>` for compile units, and retention gating on architecture **coverage** rather than a count. The round also corrected a sanitizer comment that claimed six UBSan sub-checks and named five, in all four places it appeared. **The universal build is preserved exactly** — same architectures, same `-flto`, same shipped bytes; `-object_path_lto` controls only where ld64 writes the LTO temporary, and no compiler, optimization or sanitizer flag changed. **ld64's runtime behaviour is CI's measurement, not this machine's**: this environment is Linux with no Apple linker, so the CMake half is verified here by reading generated link lines and the effect on a real universal link is verified by the new gates on the next macOS run. No DSP algorithm, parameter, schema, threading-model or latency change, and no first-party C++ source file was touched. 0.2.4 (2026-08-22) was **the macOS ccache validation round**. A review asked whether ccache really caches the universal `arm64;x86_64` build, since older ccache **refused** compilations carrying more than one `-arch`, and whether the CI saving claimed for it was ours or the sibling's. The concern was not folklore — ccache 3.2.5 `ccache.c:1999` bails with *"More than one -arch compiler option is unsupported"* — but it is **version-bound**: 3.3 added fat-binary support and 3.3.1 corrected direct-mode arch discrimination, and `macos-latest` installs **4.13.6**. Measured over three consecutive runs the job reports `Cacheable calls: 182 / 182 (100.0%)` with **no `Uncacheable` bucket in any log**, hits **14.29% cold → 95.60% warm** at an unchanged object count of 182 — only `.ccache` is restored, never `build/`, so ninja issues all 182 invocations every run and the drop is cache hits, not an incremental build. On timing the honest figure is the **compile phase**, the only part ccache acts on: **501s cold → 46–86s across two warm runs (415–455s, 83–91%)**. The step *total* spans 261–398s (41–63%) only because it also carries an LTO link that drifted 130s → 187s → 284s on a phase ccache never sees; an earlier draft of this round quoted the better warm sample as "397s (63%)" and has been corrected. Reproduced independently on ccache 4.9.1 with a control (`-save-temps` → `unsupported_compiler_option`) proving a real decline would have been visible, and with arch-varying controls that both miss, confirming the `-arch` set is in the cache key. **The configuration is kept unchanged; no build flag, architecture list or cache key was touched.** What did change is the justification: the workflow, ADR-0034 and `CI_CD.md` all quoted *the sibling's* "29m44s, 16m40s of it building". Measured here, `macos` genuinely IS the critical path at **18m43s** — but **12m34s (67%)** is the four pluginval passes and only **3m53s (20.7%)** is the build step, of which ~57s is compilation and the rest an LTO link ccache cannot cache. 16m40s overstated our build step fourfold. The round also closed the gap that made the question hard to answer at all: `macos-intel`, the single-arch control, restored a cache and never reported what it did — it now has a read-only statistics step, **added this round so not yet run** and never load-bearing anyway, since the decisive control is cold-vs-warm on the universal lane itself (`sanitizers` and `realtime` share the gap and were left alone as Linux jobs outside this question). **Proposed, not applied — gated:** `CMakeLists.txt:280`/`:336` carry `ANABASIS_BUILD_NUMBER` as a `PUBLIC` definition whose value changes every CI run, so direct mode misses on every TU that sees it and only preprocessed mode can hit — which is exactly the observed 58-direct/116-preprocessed split, the direct hits sitting in `AnabasisTests`, the one target that does not define it. Its only consumer is `src/gui/PluginEditor.cpp:210`; narrowing it there would leave shipped bytes identical, but it is a Build System change under the Architecture Review Gate. Two stale claims were corrected: the changelog's own entry count, and a `CI_CD.md` row still asserting the macOS jobs were "deliberately not cached" while two other rows in the same file said otherwise. **Carried forward and now closed:** run 32568563583 completed `linux-lto-tests` end to end under g++ 16.2.0 — both LTO links, gate clean, 301 + 873 passing — so 0.2.3's "compile phase at 16, link phase at 14" no longer applies. **Filed for the owner, not fixed here:** the arm64 slice of the shipped macOS bundle has **no dSYM** (`warning: no debug symbols in executable (-arch arm64)` for VST3, AU and Standalone; the retained `lto-objects/lto.o` is x86_64-only and the retention assertion gates on `COUNT -eq 0`, so it cannot see it). No DSP algorithm, parameter, schema, threading-model or latency change, and no first-party source file was touched. 0.2.3 (2026-08-22) was **the GCC 16 warning-gate validation round**. 0.2.2 shipped the `gcc:16` container lane having never run it, and said so; the first push ran it and it failed — not on a warning but on `fatal error: X11/extensions/XInput2.h: No such file or directory` in three JUCE translation units, ten seconds in, before the gate executed at all. JUCE 9.0.1 defaults `JUCE_USE_XINPUT` to 1, so that include is unconditional in practice, and the header belongs to **`libxi-dev`** — a package the list never named, which reached the `full` profile transitively through `libgtk-3-dev`'s `Depends: libxi-dev`. That is why every runner and every developer machine compiled it anyway, and why `headless` — which drops the gtk/webkit pair on correct evidence, since nothing here compiles webkit — took the X-input headers with it. It is now explicit on both profiles. **The warning question itself came back clean: GCC 16 introduced no new diagnostics in this tree.** The failing run still compiled all **13 first-party translation units** of the two suites under g++ **16.2.0** at `-O3 -flto -std=c++23` with the full `juce_recommended_warning_flags` set, and none emitted one — the zero-warning policy measured at 14.2.0 holds at 16, across two majors and a change of distribution, so nothing was weakened. The 43 ccache hits in that run do not soften the reading: ccache replays stored stderr on a hit, measured here in both directions, and `COMPILERCHECK=content` means a hit requires the byte-identical compiler. What that run did **not** reach is the LTO **link**, where `-Wodr` and `-Wlto-type-mismatch` fire; that phase is measured locally at GCC 14.2.0 over both suites and is likewise clean, with 301 + 873 checks passing against the LTO codegen. Compile phase measured at 16, link phase at 14. The round also stopped the gate attributing GCC findings to Clang (`--compiler`, passed by all three lanes) and pinned the driver-level `lto-wrapper:` / `cc1plus:` / `ld:` forms as non-diagnostics in the self-test (15 → 18 cases). The durable finding is methodological: 0.2.2 verified that every package name **resolves**, which is not the same question as whether the set is **sufficient** — the check that asks the second one enumerates the system headers the vendored tree includes, maps each to its owning package with `dpkg -S`, and diffs against the declared list (fifteen headers, eight packages, one omission). No DSP algorithm, parameter, schema, threading-model or latency change, and no first-party source file was touched. 0.2.2 (2026-08-22) was **the CI/toolchain parity round**. The owner's brief asked whether the migrated workflow is actually EQUIVALENT to the sibling's CI architecture rather than structurally similar; the audit read both `.github/` trees, both composite actions, both `setup-*.sh` scripts and both CMake toolchain blocks, and found the action pins, runner images, triggers, permissions, C++ standard and the whole Clang install path already equivalent — and **twelve mismatches in the configuration AROUND the toolchain** ([ADR-0034](architecture/design-decisions/ADR-0034-ci-toolchain-parity.md)). The substantive four: the **`sanitizers` gate deepened** from `address,undefined` to seven sub-checks with `detect_leaks=1` (measured — and with `unsigned-shift-base` deliberately excluded, because it fires once, on the dither RNG's xorshift, where the wrap IS the algorithm); **GCC moved 14 → 16 in the official `gcc:16` container** with the `headless` dependency profile, reversing 0.2.1's apt argument, which had chosen the version to fit the acquisition method; the **ccache contract** (`CCACHE_DIR`/`MAXSIZE`/`COMPILERCHECK=content`) adopted whole, with **the macOS jobs now cached** — the `dsymutil` objection that kept them uncached was refuted, not outvoted; and the **MSVC toolset recorded with its ABI series asserted**, closing the last platform whose shipped bytes named no compiler. Every job gained a timeout and every Linux job prints the toolchain it resolved. No DSP algorithm, parameter, schema, threading-model or latency change, and **no first-party source file was touched**. 0.2.1 (2026-08-22) was **the sibling-alignment round**, taken on the owner's directive that the two products stay aligned on engineering infrastructure unless there is a concrete technical reason to differ. Two verdicts the 0.2.0 audit had taken were re-opened under that rule and both reversed. **A2-31 (NOT NEEDED → ADOPTED):** the shipped Linux VST3 and Standalone are now built by the **pinned Clang** ([ADR-0032](architecture/design-decisions/ADR-0032-linux-release-toolchain.md)) rather than by whatever `g++` the runner image carried — ADR-0031 had pinned the compiler that gated *diagnostics* while the one that produced the *binary* floated — and `linux-clang` is deleted, its portability canary, warning gate and engine reproduction absorbed into `linux`, so the two instruments written for INC-004 now validate the bundle that ships instead of a second Clang build that was discarded. The declared **ABI floor is unchanged** on the new artifact (GLIBC 2.38 / GLIBCXX 3.4.31 / CXXABI 1.3.9), so no supported system moves. **A2-32 (INVESTIGATE/deferred → IMPLEMENTED):** `linux-lto-tests` builds both suites with `-flto` on the compile AND the link and runs them against that codegen ([ADR-0033](architecture/design-decisions/ADR-0033-lto-validation-lane.md)), on two arms — the pinned Clang (the shipped optimization class, which is the configuration INC-004 needed in order to stay hidden behind green console gates) and a **pinned GCC** from the distribution archive, which after ADR-0032 is the only place a GCC this repository chose compiles this tree (the image's unpinned `g++` still builds the `sanitizers` job's valgrind copy). Both arms gate first-party warnings at zero; measured, both suites pass identically with and without LTO under both compilers. No DSP algorithm, parameter, schema, threading-model or latency change, and the frozen registry snapshot needed no re-freeze. 0.2.0 (2026-08-22) was **the engineering-standard round**, executed from the round-2 Anamorph→Anabasis migration audit. **No DSP algorithm changed, no parameter was added, renamed or removed, and no serialization schema, threading model or reported latency moved**; the frozen registry snapshot needed no re-freeze. The minor bump is for two things a contributor has to install: the C++ baseline moves **20 → 23** ([ADR-0030](architecture/design-decisions/ADR-0030-cxx23-language-standard.md), superseding ADR-0008 decision B5's standard, closing OQ-006, and retiring both the `ANABASIS_CXX_STANDARD` seam and the weekly canary workflow), and the Linux CI **Clang major is pinned at 22** ([ADR-0031](architecture/design-decisions/ADR-0031-clang-toolchain-pin.md), installed from apt.llvm.org with the signing key pinned by identity). The substance of the round is that the repository's **Priority-1 realtime policy stopped being enforced by review alone** ([ADR-0029](architecture/design-decisions/ADR-0029-realtime-enforcement-strategy.md)): an allocation guard armed around `AnabasisEngine::process` and compiled into the DSP suite (the tier that reaches MSVC, where no sanitizer runs), a **RealtimeSanitizer** job behind a liveness canary, and a function-scoped static lint for the branches neither runtime tier executes — measured at **0 allocations over 2,040 armed `process()` calls across 80 configurations**, with the DSP suite running violation-free under RTSan and the annotation proved to produce a **byte-identical object**. `REALTIME_SAFETY_AUDIT.md`'s first Gaps entry, open since P2 and asking for exactly that instrument, is closed by name. Three more artifact properties became measurements rather than assumptions: the shipped Linux binaries now carry a **declared, gated ABI floor** (GLIBC 2.38 / GLIBCXX 3.4.31 / CXXABI 1.3.9 — the compatibility matrix previously said no Linux floor had been "decided or measured", and the artifact had one all along); **macOS crash symbolication actually works** now that `-Wl,-object_path_lto` keeps the object `dsymutil` needs, with two assertions turning a best-effort capture into a contract; and the channel probe **refuses to print a baseline it has not shown to be discriminating**, which immediately found that two of its own configurations are the same experiment twice. Three gates that could pass while blind were repaired — `check-docs.py` tested its skip list against the ABSOLUTE path and could report `0 file(s) clean` as a PASS, the portability scanner could be silently blinded by four literal shapes, and the release-notes extractor could truncate a release at a nested code fence. Every lint now ships a `--self-test` that runs in the job it vouches for (`TESTING_POLICY.md` rule 5), every GitHub Actions ref is **SHA-pinned**, and a `merge-check` job builds the one tree no push build ever compiled. 0.1.6 (2026-08-21) was **two field-report fixes on display surfaces**, no DSP, parameter-surface, schema, threading or latency change in the round. (a) The GR history's trace mapped reduction through `jlimit (0, 1, -grDb / 12) * 0.5`, so it saturated at 12 dB across HALF the panel height and drew one flat line for anything deeper while the per-stage GR meter beside it — 24 dB, same screen — went on filling; the two readouts now share `abgui::meters::grSpanDb` and the mapping moved into the pure `GrHistoryView::grY`, which is what finally made it reachable by a test. Nothing already visible moved: 24 dB over the full height puts 12 dB at half height, exactly where the old form put it. (b) The percent value boxes read a bare number in (0, 1] as a FRACTION (`0.5` → 50 %, `1` → 100 %) with an explicit `%` always literal (`0.1%` → 0.1 %), and a fractional percent now PRINTS its decimal — the integer-only formatter showed `0 %` for a value it had just accepted, and `getValueForText(getText(v))` returned 0 for it. No parameter ID, range, default or automation flag changed and the frozen registry snapshot needed no re-freeze. 0.1.5 (2026-08-16) was **the framework bump**: the JUCE pin moves 9.0.0 `f8f8864…` → **9.0.1** `e18f7f5…` ([ADR-0028](architecture/design-decisions/ADR-0028-juce-901-pin.md), a Build System change under the Architecture Review Gate, taken on the owner's directive). No first-party source changed to accommodate it and none needed to — `juce_dsp`, `juce_audio_basics`, `juce_audio_processors` and the format wrappers are byte-identical between the two upstream tags apart from a version string, and the shipped bundle's per-channel output over 33 configurations matches the old pin to nine decimal places. The bump also **ends the shared pin with Anamorph**, which stays at 9.0.0 and is read-only from here; the divergence is recorded as an open obligation in `DEPENDENCY_POLICY.md` rather than resolved. Upstream's own Linux fixes ride along and are recorded against KI-012 as candidate mechanisms now closed — **not** as a closure. 0.1.4 (2026-08-13) was the installer and interaction round: the Linux install transaction with a per-user default, the macOS package's install-state independence (INC-005), the pop-up dismissal shield and pop-up lifetime work, the tooltip gate, menu measurement and disabled-row rendering, and two stored-state BEHAVIOUR fixes inside the unchanged schema (a no-op preset apply mints no step and keeps redo; a payload-less stored A/B slot resolves to defaults as a whole). No parameter added, renamed or removed; no schema, threading, DSP-order or latency change. Two new Known Issues, KI-013 and KI-014, are filed rather than fixed. 0.1.3 (2026-08-09) was the polish round — its summary previously read "display/naming fixes only, no DSP change", which its own CHANGELOG entry retracts in the same round (that entry ships one audio-path edit, a numerical guard in the colour stage); the wording is corrected here rather than left standing in the document a returning maintainer reads first. 0.1.2 (same date) was the field-fix round (ADR-0023), which also released the PR #12 preset-identity work that sat in `[Unreleased]`. 0.1.1 remains the **first release entry** — written, dated and complete, but **not tagged and therefore not shipped** (the Release Status row below, and `COMPATIBILITY_POLICY.md` §"When the contract starts", carry that fact). v0.1.0 was declared code-complete on 2026-08-02 but never tagged either: the post-v0.1.0 review rounds landed first and the owner cut the release one patch level on, so 0.1.1's notes are the whole P1–P6 development plus the 0.1.1 round. Tagging `vX.Y.Z` drives `release.yml` (ADR-0021); publishing the resulting draft stays a human action. This row said 0.1.1 was "the first release this repository cuts" while the row below said nothing had left the repository — two documents of record disagreeing about whether anything had shipped, which is the one fact the compatibility window is keyed on. |
