...
**Gate: suites 248 + 478 = 726, `check-docs.py` clean over 72 files, pluginval both modes ×3 at
`build.yml`'s strictness with the editor under xvfb.** The one thing local evidence cannot cover
is stated in ADR-0021 rather than implied: **no installer has been built anywhere** — ISCC and
`pkgbuild` live only on the Windows and macOS runners, so the first real evidence for those two
steps is the first CI run on this branch.
