...
| Field | Value |
|---|---|
| **Current Version** | **0.2.12** (`project(Anabasis VERSION 0.2.12)` in `CMakeLists.txt`), with a dated `## [0.2.12] — 2026-09-05` CHANGELOG entry — **the owner's third GR-history report: 0.2.11 had not removed the visible instability at the right edge.** The owner saw a 1–2 px horizontal stub at the right edge of the trace and of the grey level history, jumping as the content behind it was still being generated. Reproduced on the same scratch harness (real processor, real `GrHistoryView::tick`/`paintHistory`, simulated host and frame clock) and the reading confirmed before any change: every drawn vertex was frozen and rigid, as 0.2.11 claimed, but the lead-out — the flat strip from the newest complete vertex to the plot edge, the placeholder for the bucket still collecting — was on screen, 1.00–2.42 px long at 48 kHz / 512 (2.90 at 1024), and once per bucket its height jumped and the segment left of it (up to a pitch wide) snapped from flat to sloped, the one part of the trace that changes other than by scrolling, shared by the stroke and the fill because both run the same path. Fixed by a clip and nothing else: `GrHistoryView::visibleRight` = `floor (right − min (pitch, span / 2)) − 1`, the first column `paintHistory` no longer shows — left of everything the lead-out can touch (the newest drawn vertex satisfies `right − pitch ≤ x ≤ right` on every frame), with one further column of measured margin for the stroke join at the vertex before it, which re-shapes in the frame the newer one appears. The anchor, `bucketX`, the values, the timing, the smoothed head, the host-delivery behaviour and the left edge are untouched; every column still shown is pixel-identical to 0.2.11 in 480 of 480 frames on every configuration run; the plot gives up `ceil (pitch) + 2` columns on the right (four for every block up to 1024 samples at every rate from 44.1 kHz and 2048 from 48 kHz up, on either well). Validated old against new on the same frames: in the rightmost 24 visible columns the translation-compensated movement of the fill's top edge fell from a mean of 0.52 px (max 25) to 0.07 px (max 0.44), the stroke's from 0.24 px (max 18) to 0.04 px (max 0.16) — the interior's own floor — with the last visible column lit on every frame and every column beyond it background on every frame; no stroke pixel appears or vanishes frame to frame in the three columns inside the boundary; the identity holds through a 26 s settled-window run; at 75–200 % UI scale the strip stays ≥ 0.89 px clear of the visible range. **The owner's layout call then moved the panel's cost off the panel**: the boundary was taking its four columns out of the plot, leaving the GR history four pixels narrower than the spectrum view of the same well, so `paintHistory` now draws its frame `hiddenColumns` further right (same width, same pitch — moved, not scaled), the boundary lands on the plot's own right edge, and the freed columns on the left are filled with earlier history by the `leadBuckets` extra buckets the window covers (96 ms more than twenty seconds on the Simple well, 168 ms on the Advanced). Painted columns 10…909 → 10…913 (Simple) and 10…609 → 10…613 (Advanced), the plot area being 10…913 / 10…613; every rendered column is the previous build's translated by exactly four pixels (mean difference 0.00004 px over 1.4 M samples). **The owner's view-switch report is answered by a publication on becoming visible**: the pair a frame draws (`paintHead`) is published by the tick, and nothing ticks while the spectrum owns the well, so it went stale by an entry per block that arrived meanwhile; JUCE's repaint of the newly visible view can be dispatched before the clock's first callback, and when it was, the first visible frame drew the pre-switch history — the trace and the level fill shifted right by one entry-pitch per missed entry (2.2 px after 50 ms hidden, 91.7 px after two seconds at 48 kHz / 512) — snapping back on the next frame. `visibilityChanged` now re-derives with a tick carrying the seconds the view was hidden (`abgui::HiddenInterval`), which `smoothedHead`'s clamp resolves to the live head where the producer advanced and to the parked phase where it did not, before any repaint can read it — NOT to phase 0, which would move the trace right by the offset it discarded: 148 of 248 transitions drew a stale frame before, 0 of 248 after, including 200 with no recovery time between them, and the drawing path is untouched (the right-edge validation table re-runs byte-identical). **The PR review's second finding is answered by a bucket-aligned read window**: the 20 s window is a LENGTH, so `head − window` fell mid-bucket and the paint clamped the OLDEST drawn bucket's span to it — that bucket lost its earliest entries one at a time and its value changed while it was still drawn, re-shaping the segment that crosses the left edge (340 changes in 1800 frames at 48 kHz / 512, up to 1.53 dB — 5.9 px Simple, 15.5 px Advanced — and never on any other drawn bucket, in 3.9 M readings). `Buckets::first` is now `kFirst · stride`, the oldest drawn bucket's own first entry, so every drawn bucket aggregates its complete span for its whole visible life: 0 changes in every configuration afterwards, and the left-hand columns fall to the floor that a stride-1 geometry — which cannot have the defect — shows before and after (0.080 / 2.23 px Simple, 0.259 / 9.93 px Advanced). `kFull` is capped so the alignment fits the ring (costing a saturated window one bucket), and a bucket the producer has lapped into is dropped rather than truncated (`firstDrawn`). **The `span / 2` cap answers the PR review's first finding**: `buckets` floors `kFull` at 2, so a window of two entries or fewer (`want ≤ 2`, i.e. `blockSize ≥ 10 · sampleRate` — an offline render's block) reported one pitch as the whole plot and the uncapped boundary landed one column left of the left edge, blanking the history on 100 % of frames at 48 kHz / 480000 and 44.1 kHz / 441000 on both wells. `pitch == span / 2` exactly at `kFull == 3`, so the cap engages only at `kFull == 2`, holds the boundary where a three-bucket window puts it, and leaves every other geometry bit-identical (verified: every rendered column identical before and after the cap on six configurations from 512 to 479999-sample blocks, 480–720 frames each). pluginval at strictness 10, both modes ×3, green locally. **Round 14 then implemented OQ-017's FIRST half on the owner's instruction, after re-deriving it**: a history entry was one `processBlock` CALL while the view maps entries through the PREPARED `(rate, block)` pair, so any host not delivering its declared maximum — which JUCE's own `prepareToPlay` contract says to expect, and which the AU wrapper guarantees on a live Logic track — ran the display's whole time base out by `B / D`, measured from 0.125× to 8× (at 48 kHz/512: 128 delivered gave 375 entries a second and a five-second window, 4096 gave 11.72 and 160 s, against a design 93.75 and 20 s). An entry is now one PREPARED block of PROCESSED AUDIO: the engine's chunk loop breaks on that boundary as well as on `maxBlock`, folds each entry's two statistics over the chunks inside it and pushes there, carrying the remainder across calls (`AnabasisEngine::setGrHistorySink`; the wrapper's push is gone). Cadence is `rate / B` at 44.1/48/96 kHz for 512 and the AU's 1156 across D/B = 0.25…8 (thirty configurations); the entry sequence is bit-identical under six delivery schedules including one whose every delivery crosses a boundary; each entry's peak equals the max of exactly the B rendered samples it spans, derived from the input and the group delay; a 64-sample transient inside a 4096-sample delivery lands in exactly ONE entry; and D == B is bit-identical to the previous build's pushed pair. Nothing in the display, the geometry, the smoothed head or the reader contract changed, and the GR meter stays a per-call reading. ADR-0011's "one release-store per block" clause is narrowed to say WHICH block (dated amendment, owner-instructed, flagged as a gate item). **OQ-017's bursty half remains Open** and is not partly answered by this: a burst delivers the same entries at the wrong instants. One finding recorded on the way: §5.4's `finishBlock` runs once per CALL, so the DELIVERED size sets the adaptation cadence — worth 0.0032 dB of GR across six schedules, bounded by a test rather than left to be found. **Round 15 then answered the PR review's blocking finding on that fix**: `AnabasisEngine::prepare` dropped the partial history entry on EVERY re-prepare, while `GrHistoryBuffer::prepare` keeps the ring's entries at an unchanged `(rate, block)` pair — so a transport start, the commonest host event there is, lost up to `preparedBlock − 1` samples of already-rendered audio from a timeline that went on running. Measured on the real engine and a real ring: forty pause/resume cycles at 48 kHz / 512 over 8.783 s of audio published 800 entries where the audio was worth 823 — a quarter of a second of history — and a marker burst inside the partial reached no entry at all while the ring kept every entry around it. Nothing on screen moved when it happened, because a same-pair `GrHistoryBuffer::prepare` is a total no-op: no epoch, no rewind, no slot touched, so no reader guard had anything to fire on. The partial now follows the ring — carried across exactly the re-prepares that keep its entries, dropped across exactly the ones that clear them, from the same comparison on the same two raw values (the engine keeps `preparedRateRaw`/`preparedBlockRaw` rather than its railed `sr`/`maxBlock`, so the two predicates are provably one function). `reset()` no longer clears it either — it touches no ring state, so a drop there would be the same loss at a different door, and the accumulator now has one writer rather than two that contradicted each other (round 14 cleared it twice per prepare). Nothing a host can observe changes: the wrapper does not override `AudioProcessor::reset()`. The repair removes a conflict with ADR-0023 item 6 and with `USER_MANUAL.md`'s promise rather than creating one, so it is not a gate item, but it changes a behaviour round 14 documented and test-pinned and is filed as an ADR-0011 amendment with a registry row. One bounded residual, stated: the first entry after a resume arrives up to 10.6 ms early at 48 kHz / 512, which `smoothedHead`'s clamp absorbs as at most one entry pitch of travel in a single frame — the magnitude OQ-017 already accepts for a two-block burst, against a content loss that was permanent and cumulative. The review's second, non-blocking finding is corrected with it: `GrHistoryBuffer::push` still promised one publication per `processBlock`, and the same stale unit appeared in the ring's banner twice, in `THREADING_POLICY.md`'s Audio → GUI row and in `prepareToPlay`'s comment. **Round 16 answered the review's second blocking finding on the same fix**: round 15 had the engine MIRROR `GrHistoryBuffer::prepare`'s clear-on-change comparison, which agreed for `prepare` and for nothing else. `GrHistoryBuffer::reset()` restarts the ring's timeline without going through `prepare` at all, so a partial accumulated under the old timeline survived into the new one — measured at 48 kHz / 512 with 511 samples in flight, the new timeline's FIRST entry closed on one post-reset sample and carried the previous timeline's peak. `resetGuard` moves on every `clear` and on nothing else, so the engine now records the epoch its partial was accumulated under and discards the partial when they differ — on the HOST thread, immediately after the call that may have cleared it, long before any audio is folded; the mirrored predicate is deleted and there is one definition of "which timeline is this", owned by the object that has one. Every case follows from it instead of being enumerated: a same-pair re-prepare is a ring no-op so the partial is kept (round 15's pause/resume repair, unchanged), a changed pair clears so it goes, a reset that clears goes, a reset that clears nothing stays. The read was first implemented on the audio thread, once per `process` call — airtight against callers, but at best arguable against `THREADING_POLICY.md`'s "no reads off the message thread" bullet for a scope/GR ring, whose table makes any path not in it a gate item; it moved to the thread that WROTE the epoch, with processing suspended, where it is not a cross-thread access at all. No new atomic, ordering or path, nothing added to the audio thread, one load per re-prepare. The cost is that a clear must be followed by the sync: one site in production, with the obligation stated in `GrHistoryBuffer`'s header beside both functions that can clear. Reachability is recorded rather than implied: against round 15 the leak was LATENT, since the wrapper does not override `AudioProcessor::reset()` and `GrHistoryBuffer::reset()` has no production caller — which is the argument for changing the design rather than patching the symptom, because round 15's correctness depended on that accident and restoring the clear inside `AnabasisEngine::reset()` would not have fixed the measured case at all. **Round 17 answers the review's "small buffers erase most history"** ([ADR-0040](architecture/design-decisions/ADR-0040-gr-history-ring-capacity-is-a-duration.md)). An entry is one PREPARED block, so `GrHistoryBuffer::kSize` decides `kSize · block / rate` SECONDS — and the ring was sized at 4096 against a 512-sample block, its banner generalising "~43 s at 48 kHz" to "every rate the product supports". Measured on the real engine, the real ring and the real view before any change: 20 s held only at or above 48 kHz / 256, then 10.92 s at 48 kHz / 128, 5.46 s at 48 kHz / 64, 2.73 s at 48 kHz / 32 and 0.6825 s at 192 kHz / 32 — below DESIGN §2.9's ten-second floor, not merely below `kWindowSeconds`, and against a USER_MANUAL promise the previous round had deliberately strengthened. The entry-count saturation was recorded in five places and its DURATION in none. The capacity is now 131072 entries, DERIVED (the 120000 a 20 s window needs at 192 kHz / 32, the cell ADR-0020 §1 and ADR-0011's cost note already price this product against) rather than picked, and the slots moved to the heap because a megabyte-sized member array and a Windows main thread's megabyte of stack would have met in the suites, which build rings and whole processors as locals — two of the latter live at once in places, eight in one function. Producer-side decimation was rejected as a conflict with ADR-0011's amendment — it is what an entry MEANS — and re-sizing at `prepare` as a reclamation protocol this ring deliberately does not have. Re-measured after: 20.00 s at all twenty-six pairs swept, 192 kHz / 32 included. Costs +0.87 MiB per instance (a prepared processor at 192 kHz / 32 goes from 2584 to 3476 KiB of RSS), 2.11 ns per published entry against 2.1–2.2 before, 85.7 µs for a `prepare` that clears, and a GUI decimation scan that grows with the WINDOW rather than the capacity — 163.7 µs a frame at the widest, 2.05 % of one core at the frame clock's cap, paid only by the configurations that need it. Six state assertions that pinned the OLD saturation point were re-aimed at pairs that still saturate, and `grBuckets`' count bound corrected: `kFull` reaches `cols` exactly when the window divides it. The previous entry, 0.2.11, and its investigation stand as recorded below. |
