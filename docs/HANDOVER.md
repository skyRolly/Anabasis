...
| **Test Status** | **34 checks green on Linux**: `AnabasisTests` (14 — null-with-defaults bit-exact, impulse-at-allowance for four lookahead values, ceiling clamp, hostile-input finiteness, bypass null) and `AnabasisStateTests` (20 — registry snapshot vs the frozen fixture, 49/9 counts, raw-exact byte-identical round-trip, corrupt/foreign no-op, macro fixed point, A/B tier behaviour). **pluginval L5 green ×3 in both modes on Linux** — the P1 exit criterion holds locally; 3-platform confirmation is the first CI run. |
