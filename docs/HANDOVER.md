...
| **Known Blockers** | **Three items block P1**, all decisions rather than code: (1) owner sign-off on `DESIGN.md`; (2) **OQ-010** — does the limiter lookahead get an explicit 0/off position? It must be settled *before* the parameter is created, because widening a range afterwards re-scales every saved session's normalised value and is an `ARCHITECTURE_REVIEW_GATE` item; (3) **OQ-011** — the macOS deployment target, inherited unexamined and carrying a `TODO(P1)` in `.github/workflows/build.yml`; it is a user-visible support claim, so it is decided rather than guessed (C7). The JUCE pin (OQ-001) and the plugin identity codes (OQ-003) are **decided** and recorded; both must be written into `CMakeLists.txt` when it is created at P1. The JUCE licence tier (OQ-002) blocks commercial distribution but not development. This row must agree with every `Blocking P1` entry in `docs/OPEN_QUESTIONS.md` — check it there, not here, when adding one. |
