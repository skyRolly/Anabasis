...
| Field | Value |
|---|---|
| **Current Version** | **0.2.9** (`project(Anabasis VERSION 0.2.9)` in `CMakeLists.txt`), with a dated `## [0.2.9] — 2026-09-02` CHANGELOG entry — **a stored document can no longer put a control into a state it cannot get out of.** Found by an audit of the repository's GitHub Code Scanning surface, and found by READING rather than by a scanner: nothing reports it, because the code looks like it already clamps. It does not. Every clamp the value meets is comparison-based — `juce::jlimit` in `reassertFromRaw`, `NormalisableRange::snapToLegalValue` in `applyOnePresetValue`, and Steinberg's own `Parameter::setNormalized` last of all (`VST3_SDK/public.sdk/source/vst/vstparameters.cpp:62-71`) — and every comparison against a NaN is false, so a NaN passes all three untouched and leaves the plugin through `performEdit`. JUCE's number reader returns one for the literal `nan` (`juce_CharacterFunctions.h:254-265`), so a hand-edited or half-written document is the whole delivery mechanism. Measured against the code before the fix, by linking a probe to the real plugin: a crafted session document left ALL 50 parameters non-finite, 31 readouts printing "nan", and the next save wrote all 50 back — the corruption propagated with the file. A crafted preset reached 15/50 (the NaN third; infinities clamp correctly and still do). **This is not a schema change and was not gated as one**: no field is added, removed or re-meant, and an unusable `raw` takes the same fallback `SERIALIZATION_REGISTRY.md` §1.1 already gives an ABSENT one — the read rule `AdaptiveEngine::injectTrims` and `setLearnedTargets` have applied to their own deserialized numbers since ADR-0014, and which §5's tolerance table now states for the whole schema. The audit's own first answer was that this WAS gated; the registry is what overturned it. Audio was never affected (140 blocks across five configurations, all finite), so nothing about the sound moves. Trail: `docs/architecture/SERIALIZATION_REGISTRY.md` §1.1 and §5. |
