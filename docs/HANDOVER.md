...
| **Build Status** | **No build exists.** There is no `CMakeLists.txt` and no `src/`. The three build/analysis workflows are guarded by a `preflight` job and self-skip until `CMakeLists.txt` appears — so a pre-P1 run is **not** all-skip: the **`docs` job** (`scripts/check-docs.py`, in `build.yml`) sits outside that gate deliberately and runs on every push, because pre-P1 the documentation *is* the deliverable. It gates nothing (`docs/procedures/CI_CD.md` §Build matrix). |
