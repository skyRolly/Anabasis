...
- OS Off: within tolerance at Transparent with Transients ≤50 %; +0.1…+1.2 dB over with Punchy or Transients ≥75 %.
...
- Its TP detection is disabled at ≥4x (AnabasisEngine.cpp:656).
...
**Corrections to the candidate claim.** 1. '~34 ms' is the floor, not the typical value. The bottom holds until the next host-block top, so the dropout grows with the host buffer: ≈38 ms at 512, 49 ms at 1024, 70 ms at 2048 (48 kHz).
