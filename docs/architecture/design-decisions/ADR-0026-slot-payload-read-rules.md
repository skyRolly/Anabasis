...
Two `SLOT`-reading rules changed in 0.1.4, at `src/PluginProcessor.cpp:1823` (the stored slot) and
`src/PluginProcessor.cpp:1842` (the active slot). Both were made to close a real defect, pinned by
...
- **Revert both gates.** Two expressions carry the decision:
  `stored.getChildWithName ("ANABASIS").isValid()` at `src/PluginProcessor.cpp:1823`, and
  `live.isValid() && liveSurfaceRestored` at `src/PluginProcessor.cpp:1842`. Removing them restores
...
## Related code
- `src/PluginProcessor.cpp:1823` (the stored-slot guard)
- `src/PluginProcessor.cpp:1842` (the active-slot metadata gate)
- `src/PluginProcessor.cpp:1779` (the `liveSurfaceRestored` flag both read)

Evidence [Verified]:
- Source: `src/PluginProcessor.cpp:1823`, `src/PluginProcessor.cpp:1842`
