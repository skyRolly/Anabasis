...
Two `SLOT`-reading rules changed in 0.1.4, at `src/PluginProcessor.cpp:1919` (the stored slot) and
`src/PluginProcessor.cpp:1938` (the active slot). Both were made to close a real defect, pinned by
...
- **Revert both gates.** Two expressions carry the decision:
  `stored.getChildWithName ("ANABASIS").isValid()` at `src/PluginProcessor.cpp:1919`, and
  `live.isValid() && liveSurfaceRestored` at `src/PluginProcessor.cpp:1938`. Removing them restores
...
## Related code
- `src/PluginProcessor.cpp:1919` (the stored-slot guard)
- `src/PluginProcessor.cpp:1938` (the active-slot metadata gate)
- `src/PluginProcessor.cpp:1875` (the `liveSurfaceRestored` flag both read)

Evidence [Verified]:
- Source: `src/PluginProcessor.cpp:1919`, `src/PluginProcessor.cpp:1938`
