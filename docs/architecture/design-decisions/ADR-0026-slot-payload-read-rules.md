...
Two `SLOT`-reading rules changed in 0.1.4, at `src/PluginProcessor.cpp:1801` (the stored slot) and
`src/PluginProcessor.cpp:1820` (the active slot). Both were made to close a real defect, pinned by
...
- **Revert both gates.** Two expressions carry the decision:
  `stored.getChildWithName ("ANABASIS").isValid()` at `src/PluginProcessor.cpp:1801`, and
  `live.isValid() && liveSurfaceRestored` at `src/PluginProcessor.cpp:1820`. Removing them restores
...
## Related code
- `src/PluginProcessor.cpp:1801` (the stored-slot guard)
- `src/PluginProcessor.cpp:1820` (the active-slot metadata gate)
- `src/PluginProcessor.cpp:1757` (the `liveSurfaceRestored` flag both read)

Evidence [Verified]:
- Source: `src/PluginProcessor.cpp:1801`, `src/PluginProcessor.cpp:1820`
