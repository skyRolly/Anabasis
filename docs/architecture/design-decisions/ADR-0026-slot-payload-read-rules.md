...
Two `SLOT`-reading rules changed in 0.1.4, at `src/PluginProcessor.cpp:1757` (the stored slot) and
`src/PluginProcessor.cpp:1776` (the active slot). Both were made to close a real defect, pinned by
...
**Each option below was EXECUTED and MEASURED (2026-08-13), not reasoned about**, so that the
...
|---|---|---|
| Keep both (the tree as it stands) | 840 checks, **0 failures** | — |
| Revert BOTH decisions | 840 checks, **5 failures** | 3 × `malformedSlot` (including `a payload-less slot lent its preset name to another state ('Ghost Session')`) + 2 × `rootlessActive` |
| Keep decision 1, revert decision 2 | 840 checks, **2 failures** | 2 × `rootlessActive` only — the `malformedSlot` trio passes, confirming the two decisions are independently revertible and that decision 1 alone closes the reported defect |
...
- **Revert both gates.** Two expressions carry the decision:
  `stored.getChildWithName ("ANABASIS").isValid()` at `src/PluginProcessor.cpp:1757`, and
  `live.isValid() && liveSurfaceRestored` at `src/PluginProcessor.cpp:1776`. Removing them restores
...
## Related code
- `src/PluginProcessor.cpp:1757` (the stored-slot guard)
- `src/PluginProcessor.cpp:1776` (the active-slot metadata gate)
- `src/PluginProcessor.cpp:1713` (the `liveSurfaceRestored` flag both read)

Evidence [Verified]:
- Source: `src/PluginProcessor.cpp:1757`, `src/PluginProcessor.cpp:1776`
