...
| [ADR-0039](ADR-0039-spectrum-frame-publication.md) | **The spectrum publishes its two traces to the painting thread as ONE frame** — `inDb`/`outDb` were plain `std::vector<float>` written by `tick` on the message thread and read by `paint` on whichever thread paints (the GL render thread on macOS/Windows): a data race, and a frame could combine the input trace of tick N with the output trace of tick N+1 (1 161 778 of 1 321 607 reads, measured). A sequence bracket over atomic per-bin storage publishes both traces and the window they describe as one; the painter keeps its own copy and gives up after two attempts. Adds the first PAYLOAD to the Message → Painting boundary, which ADR-0027 clause 4 and ADR-0038 clause 8 both return to this gate. A reset publishes the empty frame so a reconfiguration cannot leave the old mapping on screen | **Proposed — 2026-09-06** (Architecture Review Gate OPEN; a merge prerequisite that a green build does not clear) | Verified — `specFrame`, `specReset`, five mutants |
