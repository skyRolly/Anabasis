...
| [ADR-0011](ADR-0011-threading-model.md) | Threading model: two threads, no workers, atomic + SPSC publication | Accepted | **Partially Verified** — every implemented edge is mapped in `THREAD_MODEL.md` and `REALTIME_SAFETY_AUDIT.md` audits the audio thread; the two questions it left open are OQ-014 and the off-thread `replaceState` race (KI-003), and ADR-0012 amends its permitted-path table; amended in place 2026-09-02 (0.2.8 review): the GR ring's payload is atomic, its prepared (rate, block) pair is ring metadata stored inside the clear, and its seqlock reader closes a batch with an acquire fence (`batchIntact`) — the second and third amendments were Architecture Review Gate items, **both accepted by the owner 2026-09-02 and cleared** |
...
| [ADR-0023](ADR-0023-012-field-fix-contracts.md) | The **0.1.2 field-fix contracts**: comp knee moves ABOVE the threshold (zero gain at or below `T` — the all-defaults null holds for any sub-ceiling input); the limiter's detector is **unfiltered** (`scHpfFreq` becomes comp-only, its filtered magnitude clamped to a raw-magnitude ceiling — a sidechain HPF may only deafen a detector); bus layouts gain **mono→mono**; the GR history draws at a fixed right-anchored scale with a zero-data unmeasured region and survives same-config re-prepares; the GR\|SPEC pill moves bottom-left as a whole-pill toggle with GR the default view; editor captions drop stage prefixes while the limiter's registry NAME becomes "Limiter Stereo Link" (ID unchanged, snapshot re-frozen); the Advanced macro row is removed (940×822); per-channel per-stage GR is published (the KI-009 disambiguator); **gate cleared 2026-08-09** (the owner's 0.1.2 release directive, quoted in the Status banner); **item 6 amended in place 2026-09-01 (0.2.8)** — the pitch and anchor stand, the trace now scrolls by the entry rather than by the bucket | Accepted | Verified — `testNullWithDefaults` (bit-exact null + Delta silence at defaults), `testCompStaticCurve`, `testLimiterDetectorIsUnfiltered`, the fixed-scale `grBuckets` section, `testGrRingResetEpoch`, the graph-switch click tests, `testMeterPublication`'s per-channel lanes, `testBothChannelsCarryAudioThroughTheWrapper` (mono→mono + six diagnostic configurations); eight mutants across two control builds, each killed by its own assertion |
...
| [ADR-0026](ADR-0026-slot-payload-read-rules.md) | **A `SLOT` carrying no `ANABASIS` child resolves to defaults as a whole, and the ACTIVE slot's metadata is adopted only when the ROOT surface was restored** — one rule per slot, reached by two different routes because the slots are not symmetric: metadata is adopted only alongside the parameters it describes. Closes a defect where a payload-less stored slot put one session's preset name and identity on another session's sound. No producer exists for either shape (`getStateInformation` always writes the root surface and both payloads), so the compatibility exposure is bounded to hand-edited, truncated or foreign blobs — which does NOT clear the gate. Implemented in 0.1.4 without being flagged as gated, found by review 2026-08-13; **the owner ratified it on 2026-08-14** | Accepted | Verified — `testAMalformedStoredSlotCannotSplitSoundFromMetadata` (mutant reproduces the defect verbatim), `testARootlessSurfaceDropsTheActiveSlotsMetadataToo` |
| [ADR-0027](ADR-0027-painting-thread-reads-editor-bookkeeping.md) | **A look-and-feel override may read ONE editor counter from the painting thread, through `std::atomic<int>` at `memory_order_relaxed`** — `drawResizableFrame` asks `isPopupMenuOnScreen` to tell a parented pop-up from a `ResizableBorderComponent`, and that call arrives from `paintOverChildren`, i.e. the GL render thread wherever the context attaches. Permits one scalar with no payload and no ordering role, read-only from the painting side; a hook the paint path invokes is torn down only after `glContext.detach()` joins that thread. `ARCHITECTURE_REVIEW_GATE.md` classes a new cross-thread path as gated and 0.1.4 introduced it without flagging, while the `THREAD_MODEL.md` paragraph written in the same round states the trigger verbatim. Found by review 2026-08-13; **the owner ratified it on 2026-08-14** | Accepted | Not verifiable by the suites (no headless test opens a modal menu or runs a GL render thread) — an ADR-0025 class-1 exception, defended by the `DEPENDENCY_POLICY.md` rule-7 register |
| [ADR-0028](ADR-0028-juce-901-pin.md) | **The JUCE pin moves 9.0.0 `f8f8864…` → 9.0.1 `e18f7f5…`, and stops being the sibling's pin.** A Build System change under `ARCHITECTURE_REVIEW_GATE.md` and `DEPENDENCY_POLICY.md` upgrade rule 1, taken on the owner's explicit directive (2026-08-16, version 0.1.5). Of the fifteen JUCE modules Anabasis links, **six are byte-identical between the two tags apart from their module-declaration version string, and they are the six that decide how the plug-in sounds and what a host sees** — `juce_dsp`, `juce_audio_basics`, `juce_audio_processors`, `juce_data_structures`, `juce_audio_utils`, `juce_audio_plugin_client` — so no DSP, latency, parameter or state-ABI change has a source to come from. The nine that did change are GUI, platform-native, file-format and web-view surfaces, tabulated in the ADR with each one's diff size and why it is or is not reachable here. Neither documented breaking change reaches this project (the C-mode vendored codecs were already `.c` at 9.0.0 and nothing here links a separate copy; the WebView package move is inert under `JUCE_WEB_BROWSER=0`). The one delta touching this product's own data is a **widening** of `XmlDocument`'s tolerance inside a character block, for a shape `XmlElement::toString` never emits. The bump **breaks the shared pin with Anamorph** and no work in this repository can repair that — the sibling is read-only here — so the divergence is recorded as an open obligation rather than resolved by silence | Accepted | Verified — 1141 checks green at the new pin (unchanged counts) and `AnabasisChannelProbe` agrees **digit for digit at nine decimal places** with the same source built at the old pin, across 33 configurations of the shipped VST3 bundle plus the macro-with-editor scenario; pluginval at the `build.yml` strictness, both modes ×3. The rule-2 **Level-5 audition is owed and is recorded as owed** in the `DEPENDENCY_POLICY.md` compliance log |
| [ADR-0029](ADR-0029-realtime-enforcement-strategy.md) | **Realtime-safety enforcement in three complementary tiers** — the first mechanical detector this repository has had for its Priority-1 policy. `tests/AllocationGuard.h` (replaceable `operator new` + glibc malloc interposition, armed only around `AnabasisEngine::process`, and the tier that reaches **MSVC** where no sanitizer runs); a `realtime` CI job running the DSP suite under **RealtimeSanitizer** behind a liveness canary, with `ANABASIS_NONBLOCKING` on the engine's audio entry point; and `scripts/check-realtime.py`, a function-scoped static lint that reads the branches the suite never executes. `-Wfunction-effects` is scoped to the JUCE-free leaf layer only, because JUCE carries no effect annotations and the flag would otherwise warn about correct code. Adds NO new constraint — the policy is unchanged; what changed is that breaking it now fails a job. Closes `REALTIME_SAFETY_AUDIT.md`'s first Gaps entry, open since P2 | Accepted | Verified — the annotation produces a **byte-identical object** (clang-22, -O3, project flags, live vs emptied: 105,224 bytes, same MD5), the DSP suite runs **violation-free under RTSan**, the canary aborts with the RTSan signature at exit 43, the guard finds **0 allocations over 2,040 armed `process()` calls across 80 configurations** with both counters proved live, and the effects canary fails with the expected `-Wfunction-effects` diagnostic |
| [ADR-0030](ADR-0030-cxx23-language-standard.md) | **The language standard moves C++20 → C++23**, on the owner's directive, superseding ADR-0008 decision B5's standard and closing OQ-006. The `ANABASIS_CXX_STANDARD` cache seam and the weekly `cxx23-canary.yml` are **removed**: the canary's question is now answered by every job on all three platforms as a blocking check, and an option with no caller is dead state. B5's other half — **no modules** — is reaffirmed, and C++23 *library* features remain admissible only behind feature-test macros and a first-party abstraction | Accepted | Verified — both suites green at 23 under GCC 13.3, Clang 18.1.3 and the pinned Clang 22.1.8, with **zero first-party warnings** under the full gate; no source changed to accommodate the standard |
| [ADR-0031](ADR-0031-clang-toolchain-pin.md) | **The Linux Clang major is pinned at 22** (`ANABASIS_CLANG_VERSION`, installed by `scripts/setup-llvm-apt.sh` from apt.llvm.org with the signing key pinned by identity), because the zero-first-party-warning gate had no stable reference point and RealtimeSanitizer does not exist in the major Ubuntu's archives stop at. **No warning baseline file is introduced** — the sibling's debt-list mechanism was rejected because it would *permit* classes this gate forbids. Also adds the review gate's §"Compiler and toolchain versions" rule: a version this repository pins is gated, a version the runner image supplies cannot be and is covered by detection and record instead | Accepted | Verified — clang-22.1.8 installed by the script, **no first-party warnings** across the whole Clang leg, both suites green, `libclang_rt.rtsan` present and `-fsanitize=realtime` links and runs |
| [ADR-0032](ADR-0032-linux-release-toolchain.md) | **Clang builds the shipped Linux artifact; GCC becomes the compatibility compiler.** The binary users install was previously produced by whatever `g++` the runner image carried — the one part of the toolchain nothing pinned, while ADR-0031 pinned the compiler that only gated diagnostics. `linux-clang` is deleted and its three own steps (portability compile canary, warning-gate self-test + gate, `AnabasisEngineRepro`) move into `linux`, so the two instruments aimed at INC-004 now validate the bundle that ships rather than a second build that was discarded; `merge-check` moves to the same compiler and shares its ccache lineage. GCC is pinned (`ANABASIS_GCC_VERSION`) and keeps a job in `linux-lto-tests` | Accepted | Verified — the clang-22 build passes both suites, the engine reproduction, the discriminating channel probe, pluginval ×3 in both modes, and the **unchanged** ABI floor (GLIBC 2.38 / GLIBCXX 3.4.31 / CXXABI 1.3.9), with zero first-party warnings |
| [ADR-0033](ADR-0033-lto-validation-lane.md) | **`linux-lto-tests`: both suites built with `-flto` on the compile AND the link, then run against that codegen — two arms, the pinned Clang and the pinned GCC.** By ADR-0008 the suites do not link the LTO flags the plugin does, which is precisely the configuration INC-004 needed in order to stay hidden behind green console gates for five months; the Clang arm closes that, and the GCC arm is (after ADR-0032) the only place a GCC this repository chose compiles this tree. GCC is pinned from the distribution archive rather than through the sibling's `gcc:16` container, which keeps A2-34's dependency-profile split NOT NEEDED | Accepted | Verified — 301 + 873 checks, 0 failures, under `clang-22 -flto` and `g++-14 -flto`, identical to the same suites without LTO under both compilers |
| [ADR-0034](ADR-0034-ci-toolchain-parity.md) | **CI/toolchain parity with the sibling, area by area.** The audit found the previous rounds had migrated workflow STRUCTURE while leaving the configuration around the toolchain behind. Four decisions: GCC moves **14 → 16 through the official `gcc:16` container** with the `headless` dependency profile (reversing ADR-0033's apt argument, which had chosen the version to fit the acquisition method, and re-opening A2-34); the LTO lane becomes **two jobs** because `container:` is per-job; the **ccache contract** (`CCACHE_DIR`/`MAXSIZE`/`COMPILERCHECK=content`) is adopted whole and **the macOS jobs are cached** — the `dsymutil` objection that kept them uncached was refuted, not outvoted; and the **sanitizer set deepens** by five sub-checks plus `detect_leaks=1`, with `unsigned-shift-base` excluded on a measurement. Also: the MSVC toolset is recorded and its ABI series asserted, every job gets a timeout, and every Linux job prints the toolchain it resolved | Accepted | Verified — both suites **300 + 873, 0 failures** under the adopted sanitizer set with leak detection on; the excluded sub-check and the 64 MB stack each measured in both directions. **The `gcc:16` container lane is unverified locally** (no container runtime in this environment) |
| [ADR-0035](ADR-0035-macos-per-architecture-symbolication.md) | **macOS symbolication is a per-ARCHITECTURE contract.** ADR-0034 called the dSYM capture a gate; on arm64 it was not one. A fat dSYM lists a UUID for every slice whether or not that slice carries DWARF, and the compile-unit count was taken over the whole fat file — so 39 units from x86_64 plus 0 from arm64 summed to 39, cleared `-eq 0`, and the job reported a validated dSYM while every arm64 crash log was unsymbolicatable. The retention assertion was blind the same way: its own comment expects one object per (target × slice) and it tested only `COUNT -eq 0`, reading 1. Both now gate per slice — compile units counted with `dwarfdump --arch=<A>`, and retention gating on every shipped architecture having a retained LTO object (coverage, not a count, because a count tracks the target list and was not what broke). Cause: several links share one `-Wl,-object_path_lto` path and overwrite each other, on both axes — the survivor is one target's x86_64 object | Accepted | Verified — gate logic exercised over six cases against stub `lipo`/`dwarfdump`: **arm64-empty and x86_64-empty both discard where the aggregate logic kept**, single-arch keeps so `macos-intel` is unaffected. **Behaviour against a real universal dSYM is verified in CI, not locally** (no Apple toolchain in this environment) |
| [ADR-0036](ADR-0036-parity-audit-round-2.md) | **Parity audit round 2.** The sibling's CI surface is byte-identical to ADR-0034's baseline, so the fresh 200-row read found only what that audit missed or what Anabasis changed since. Nine adoptions, each verified before landing: `MALLOC_PERTURB_` 255→1 (measured — glibc fills FRESH heap with the complement, so 255 wrote the benign 0x00 the variable exists to defeat); the build number becomes a source-file property on `PluginEditor.cpp` (2 of 103 compile commands carry it now — the 0.2.4 direct-mode finding, fixed with family precedent); macOS pluginval validates `dist/` (OQ-012's macOS half resolved — packaging before validation, AU installed from dist); the Rosetta result gates the macOS uploads; the MSVC assert moves last so a toolset move cannot destroy its own evidence; ccache zero+print lands on 8/8 cached jobs; macos-intel gains the thin-slice assert and randomise arms; three GCC-only diagnostics join the GCC lane under the zero gate (measured: 4 vendored hits, 0 first-party); PE guard +26, Windows no-cache rationale, AU cleanup. Kept-different list re-affirmed item by item; JUCE pins observed re-converged at 9.0.1 | Accepted | Verified — suites **301 + 873, 0 failures** incl. under `MALLOC_PERTURB_=1`; fill bytes, define scoping and diagnostics each measured directly. **The reordered macOS/Windows jobs are verified structurally here and behaviourally by CI's next run** (no Apple/Windows toolchain in this environment) |
| [ADR-0037](ADR-0037-llvm-23-toolchain-upgrade.md) | **LLVM 23.1.0 is released; what apt.llvm.org ships for major 23 is not — so the pin HOLDS at 22 and the install learns to tell the difference.** The directive to take the newest stable and the directive to refuse release candidates conflicted in fact. `clang-23` on noble is built from a `release/23.x` commit that still carries `LLVM_VERSION_SUFFIX -rc3`; Debian's packaging drops the suffix, so `--version`, `__clang_version__` and the dpkg version all read a clean `23.1.0`, and **every gate in this repository passed on a release candidate** because every one of them is major-only. `setup-llvm-apt.sh` now asserts the package's upstream commit IS the release tag's commit — verified to accept 22.1.8 and refuse 23.1.0. The 23 measurements are kept, so the move is one line once upstream rebuilds. Amends ADR-0031 clauses 1 and 2. | **Accepted** 2026-08-30 · 0.2.7 |
| [ADR-0038](ADR-0038-gr-history-display-scalars-cross-the-painting-boundary.md) | **The GR history's two scroll scalars cross the painting boundary** — `shownHead` and `smoothHead`, published by the frame-clock tick on the message thread and read by `paintHistory` on whichever thread paints (the GL render thread on macOS/Windows). They were plain scalars at 0.2.8, i.e. a data race on those two platforms. Made relaxed `std::atomic`s, `static_assert`ed lock-free, and — the load-bearing part — argued safe as a PAIR by value rather than by snapshot: every stale/fresh pairing resolves to `min (smoothHead, head + 1)`, a frame the ramp itself produces, so no consistency mechanism is needed. Widens ADR-0027's Message → Painting row from one scalar to scalars whose cross-pairings are legal frames; amends ADR-0027 clause 4 in place, dated | **Accepted — 2026-09-02** (Architecture Review Gate cleared on the owner's approval; filed `Proposed` and held there until answered) | Verified — `grPair` (every cross pairing over a 64-frame publication sequence; the lock-free assertion mirroring the header's `static_assert`); the race itself is not stageable and is argued, not measured |
| [ADR-0039](ADR-0039-spectrum-frame-publication.md) | **The spectrum publishes its two traces, and the configuration that makes them readable, to the painting thread as ONE frame** — `inDb`/`outDb` were plain `std::vector<float>` written by `tick` on the message thread and read by `paint` on whichever thread paints (the GL render thread on macOS/Windows): a data race, and a frame could combine the input trace of tick N with the output trace of tick N+1 (1 161 778 of 1 321 607 reads, measured). A sequence bracket over atomic per-bin storage publishes both traces and the frame description as one; the painter keeps its own copy and gives up after two attempts. **Widened 2026-09-06 before approval** to carry the SAMPLE RATE too, taken under `GrHistoryBuffer`'s reset epoch: `paint` was reading the rate for itself, so a frame could pair one configuration's trace with another's bin mapping (6 kHz: -0.00 dB paired, -116.80 and -120.00 crossed). Adds the first PAYLOAD to the Message -> Painting boundary, which ADR-0027 clause 4 and ADR-0038 clause 8 both return to this gate, and carries ADR-0038 clause 7's identity rule one step further. A reset publishes the empty frame so a reconfiguration cannot leave the old mapping on screen | **Accepted — 2026-09-06** (Architecture Review Gate cleared on the owner's approval; filed `Proposed`, widened before approval, and held there until answered) | Verified — `specFrame`, `specReset`, `specAxis`, `specPaint`; eleven mutants killed and three recorded as surviving |

ADR-0028 is the **first entry in `DEPENDENCY_POLICY.md`'s compliance log**, which had stood empty
with the note "no dependency bump has occurred". Its rule-2 cell is deliberately half-empty: the
headless verification is done and the manual audition is not, and a bump ADR that let those two
read as one thing would be the failure this index exists to make visible.
...
| **0008** | **0028** | The JUCE **version and SHA** quoted in its Decision (`ANABASIS_JUCE_VERSION "9.0.0"` / `ANABASIS_JUCE_TAG "f8f8864…"`) — now 9.0.1 / `e18f7f5…`. Everything ADR-0008 actually decided about the dependency is **reaffirmed rather than moved**: FetchContent, a pin by immutable SHA and never a tag *name* (decision A5), `GIT_SHALLOW` with its documented caveat, the `ANABASIS_JUCE_PATH` escape hatch, and the two cache variables carrying the pin. ADR-0028 is that mechanism being used, not replaced. The five-target graph, the C++20 baseline and the frozen identity are untouched. |
...
| **0027** | **0038** | Clause 4, amended in place and dated (see the ADR-0038 row above). This registry had no entry for it before 2026-09-06 — found by round 11's policy sweep and added here, because a reader auditing ADR-0027 through the table alone would have concluded nothing had moved. |
| **0027**, **0038** | **0039** | Clause 4 and clause 8, amended BY EXCEPTION and dated in both records: their boundary stands for every site they cover, and one site is added — `SpectrumView`'s published frame — where a mechanism supplies the consistency instead of an argument. The permission is for that site and that mechanism, not for payloads in general. |
