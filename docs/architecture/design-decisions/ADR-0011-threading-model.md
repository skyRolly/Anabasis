...
`Anamorph:src/PluginProcessor.cpp:553-559` [Verified]). **The trim-vector case is under-specified
...
- **The permitted-path table gains a seventh row, for a payload-carrying command atomic** *(added
  ...
  (`Anamorph:src/PluginProcessor.cpp:553-559` [Verified]). Since the policy states "Any path not in
  ...
  **is** a thread-model change and an Architecture Review Gate item.
  ...
  > establishes (`Anamorph:src/PluginProcessor.cpp:553-559` [Verified] carries a single gain), and
...
- Precedent [Verified — cited via `DESIGN.md` §1.1, §2.9, §3.3, §5.4, §6.5]:
  ...
  predicted == latched latency), `Anamorph:src/PluginProcessor.cpp:553-559` (sentinel-atomic
  per-slot inject consumed at the duck bottom), `Anamorph:src/PluginProcessor.cpp:338-489`
