...
**One call site** — `updateLatency()` (`src/PluginProcessor.cpp:887-905` — the definition through its
...
| `prepareToPlay` | `src/PluginProcessor.cpp:804` (`prepareToPlay`, its `updateLatency()` call) |
| `setNonRealtime` | `src/PluginProcessor.cpp:877-885` (`setNonRealtime`) |
...
deliberately redundant `updateLatency()` (`src/PluginProcessor.cpp:2009`, the last statement of
