...
`copyXmlToBinary` (`getStateInformation`, `src/PluginProcessor.cpp:1746-1802`). Structure, in write order:
...
holding the exact normalised position (`copyStateWithRaw`, `src/PluginProcessor.cpp:1043`;
...
(`reassertFromRaw`, `src/PluginProcessor.cpp:1327-1368`). **A `raw` that is NaN takes that same
...
`src/PluginProcessor.cpp:1062-1094`): a NaN `value` is dropped from the tree before `replaceState`,
...
via `saveSlotFromLive()` (`src/PluginProcessor.cpp:1201-1268`). Two
...
`saveSlotFromLive`, `src/PluginProcessor.cpp:1194-1197`). Three rules the shape encodes:
...
(`src/PluginProcessor.cpp:1224-1225, 1490, 1933` — `saveSlotFromLive`, `applySlotToLive`,
...
(`src/PluginProcessor.cpp:1611, 1698, 1731` — `applyFactoryPreset`, `applyPresetFile`,
...
from defaults (`getStateInformation`'s ADAPTIVE block, `src/PluginProcessor.cpp:1784-1798`). While a loaded restore is staged but
...
`setStateInformation` (`src/PluginProcessor.cpp:1804-2001`), in order:
