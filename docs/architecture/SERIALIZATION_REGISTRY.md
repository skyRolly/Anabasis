...
`copyXmlToBinary` (`getStateInformation`, `src/PluginProcessor.cpp:1646-1702`). Structure, in write order:
...
holding the exact normalised position (`copyStateWithRaw`, `src/PluginProcessor.cpp:988`;
...
(`reassertFromRaw`, `src/PluginProcessor.cpp:1255-1268`). This is what makes a mid-step position
...
via `saveSlotFromLive()` (`src/PluginProcessor.cpp:1129-1196`). Two
...
`saveSlotFromLive`, `src/PluginProcessor.cpp:1122-1125`). Three rules the shape encodes:
...
test that seeds one (`tests/state_tests.cpp:2171`). The wrapper *adopts* the child from an
incoming slot or session, *carries* it through A/B, undo and saves
(`src/PluginProcessor.cpp:1152-1153, 1390, 1833` — `saveSlotFromLive`, `applySlotToLive`,
...
(`src/PluginProcessor.cpp:1511, 1598, 1631` — `applyFactoryPreset`, `applyPresetFile`,
...
from defaults (`getStateInformation`'s ADAPTIVE block, `src/PluginProcessor.cpp:1684-1698`). While a loaded restore is staged but
...
`setStateInformation` (`src/PluginProcessor.cpp:1704-1901`), in order:
