...
`copyXmlToBinary` (`getStateInformation`, `src/PluginProcessor.cpp:1764-1820`). Structure, in write order:
...
holding the exact normalised position (`copyStateWithRaw`, `src/PluginProcessor.cpp:1061`;
...
(`reassertFromRaw`, `src/PluginProcessor.cpp:1345-1386`). **A `raw` that is NaN takes that same
...
`src/PluginProcessor.cpp:1080-1112`): a NaN `value` is dropped from the tree before `replaceState`,
...
via `saveSlotFromLive()` (`src/PluginProcessor.cpp:1219-1286`). Two
...
`saveSlotFromLive`, `src/PluginProcessor.cpp:1212-1215`). Three rules the shape encodes:
...
(`src/PluginProcessor.cpp:1242-1243, 1508, 1951` — `saveSlotFromLive`, `applySlotToLive`,
...
(`src/PluginProcessor.cpp:1629, 1716, 1749` — `applyFactoryPreset`, `applyPresetFile`,
...
from defaults (`getStateInformation`'s ADAPTIVE block, `src/PluginProcessor.cpp:1802-1816`). While a loaded restore is staged but
...
`setStateInformation` (`src/PluginProcessor.cpp:1822-2019`), in order:
