...
`copyXmlToBinary` (`getStateInformation`, `src/PluginProcessor.cpp:1605-1661`). Structure, in write order:
...
holding the exact normalised position (`copyStateWithRaw`, `src/PluginProcessor.cpp:977`;
...
(`reassertFromRaw`, `src/PluginProcessor.cpp:1244-1257`). This is what makes a mid-step position
...
via `saveSlotFromLive()` (`src/PluginProcessor.cpp:1118-1185`). Two
...
`saveSlotFromLive`, `src/PluginProcessor.cpp:1111-1114`). Three rules the shape encodes:
...
(`src/PluginProcessor.cpp:1141-1142, 1363, 1792` — `saveSlotFromLive`, `applySlotToLive`,
...
(`src/PluginProcessor.cpp:1484, 1559, 1590` — `applyFactoryPreset`, `applyPresetFile`,
...
from defaults (`getStateInformation`'s ADAPTIVE block, `src/PluginProcessor.cpp:1643-1657`). While a loaded restore is staged but
...
`setStateInformation` (`src/PluginProcessor.cpp:1663-1860`), in order:
