...
| Frozen-trim **application** generation → the saver | `frozenStageSeq` (bumped by the message-thread stager, before the record flag) vs `frozenAppliedSeq` (stored by the audio thread only after `injectTrims`), both relaxed; `frozenRestorePending()` is their inequality. The record flag alone answers "consumed?", and the save needs "**applied?**" — the ~34 ms between the block top and the duck bottom is a window in which the published trims are still the pre-restore ones, and a save landing there (the editor's ~3 Hz dirty-marker poll reaches it in ordinary use) serialised them | Audio → GUI (generation / staleness counters) | `src/dsp/AnabasisEngine.h` (`frozenRestorePending`), `src/PluginProcessor.cpp` (`saveSlotFromLive`) |
