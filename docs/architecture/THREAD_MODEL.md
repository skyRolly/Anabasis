...
| **GPU render context** | yes, macOS/Windows only | Component painting when attached — created and driven by JUCE, holds no Anabasis state (`src/gui/PluginEditor.h:566`, attach gated per platform per DESIGN §6.1) |
...
The OpenGL context attaches on macOS/Windows only, never Linux/X11 (`src/gui/PluginEditor.h:566`
