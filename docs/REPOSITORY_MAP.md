...
| `check-portability.py` | Source lint for the JUCE SIMD-overload hazard (INC-003): an explicit template argument on `{jmin, jmax, snapToZero}` resolves to the `dsp::SIMDRegister` overload on macOS and not on Linux. Carries a `--compile-canary`; mutation-verified. Run by **source-lint**. |
...
| Path | Responsibility |
|---|---|
| `workflows/build.yml` | 3-OS build + self-tests + pluginval (both modes ×3, **blocking on all three platforms**); retain-then-strip symbol pipeline; fail-closed artifact staging; also callable (`workflow_call`) by a future `release.yml`. Strictness comes from one top-level `env` and escalates by phase. Also carries the lint and analysis jobs, which run pre-P1 and have no `needs:`: **docs** (`scripts/check-docs.py`), **source-lint** (`scripts/check-portability.py` + `scripts/check-citations.py`, full-history checkout), **linux-clang** (Clang build + `scripts/check-clang-warnings.py` + both reproductions) and **sanitizers** (ASan/UBSan + valgrind). |
