...
| **1b** | Portability + memory + evidence anchors | Source lint for the JUCE SIMD-overload hazard; documentation `file:line` anchors still naming the code they were written about; **both** compilers gating on first-party warnings at zero; ASan + UBSan (seven sub-checks, leak detection on) + valgrind over both suites | `scripts/check-portability.py`, `scripts/check-citations.py`, `scripts/check-clang-warnings.py`; `build.yml` jobs `source-lint`, `linux`, `linux-lto-tests`, `sanitizers` |
| **1c** | Shipped-class codegen | Both suites built with `-flto` on the compile AND the link and run against that codegen, on the pinned Clang (the shipped optimization class) and the pinned GCC (the other major toolchain, in its own container). The gap it closes is named: by ADR-0008 the suites do not link the LTO flags the plugin does, which is the configuration INC-004 needed in order to stay hidden for five months | `build.yml` jobs `linux-lto-clang` and `linux-lto-tests` (ADR-0033, ADR-0034) |
