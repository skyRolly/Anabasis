...
9. **No NaN / Inf / denormals leave the engine, and the engine self-heals.** Denormal protection
   ...
   **A DRY/WET MIX IS THE SECOND WAY INTO THAT CLASS, and it is not obvious from the stage's own
   code.** The two stages above are unobservable because they emit no audio. A stage that DOES emit
   audio becomes unobservable the moment its mix reaches an exact dry endpoint: the mix loop leaves
   the input sample untouched, so the stage's output is finite whatever its internals did, and the
   boundary that would raise `stageGeneratedNonFinite` has nothing to see. Any state the stage keeps
   WARM across that endpoint — kept warm deliberately, so a filter does not splice against stale
   audio when it re-engages — is then a latch: it survives every block, every preset load and every
   A/B, and is paid for later, on ordinary audio, the moment the mix opens. **`ClipSat` is
   ...
   **The rule for a future stage with a mix control:** being in the `sanitiseState` list is
   necessary and NOT sufficient, because at the dry endpoint the repair is never called. Either
   bound the arithmetic so the state cannot go non-finite, or make the state repair unconditional
   for that stage the way the two invisible stages above are handled.
