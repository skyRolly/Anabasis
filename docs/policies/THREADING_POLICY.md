...
| Audio → GUI (scope / GR history) | SPSC ring buffer | Exactly **one producer thread and one reader thread**; release/acquire on the write index, published **once per COMMITTED UNIT** — one PREPARED BLOCK of processed audio for the GR history since 0.2.12 (ADR-0011 amended 2026-09-07), a processing CHUNK for the spectrum taps. Neither unit is the host's call: both publish from inside the engine, so a single `processBlock` commits none, one or several of either, depending on how much audio it delivered against the prepared size. The guarantee the wording exists for is unchanged either way: a reader that acquires the index sees every frame below it complete, because the payload is written before the index is released. Only the reader's "has anything new arrived?" cadence differs. **The PAYLOAD is atomic as well in `GrHistoryBuffer` (ADR-0011 amended 2026-09-02):** the index ordering says what a reader sees, not that a read landing on the slot the producer is writing is legal — and that read is precisely what this ring's guards exist to DETECT and discard, which is the wrong tool, since a plain read concurrent with a plain write is undefined the moment it happens. The stored fields are `std::atomic<float>`, relaxed both ways, so the racing read is defined (each field one of the two values, the pair possibly mismatched) and the guards keep their job. The audio-thread store is instruction-identical to the plain one and `static_assert`ed lock-free. **Round 6 added the PRODUCER's half of the GR ring's lap check (ADR-0011 amended a fourth time, 2026-09-02, raised at the gate and held):** `push` release-fences before its payload stores, so a reader whose relaxed peek returned a lapping push is forced by [atomics.fences] to observe that push in its closing `available()` re-read — without it the re-read could return a pre-lap index and an overtaken frame was ACCEPTED, drawn with entries the producer had overwritten. Instruction-identical on x86-64, one `dmb ish` per PUBLISHED ENTRY on AArch64. **Since the KI-015 follow-up (ADR-0011 amended a third time, 2026-09-02) the same rule covers `ScopeBuffer`, the spectrum taps' ring** — its producer wrote the payload with `memcpy` and its reader read it with plain subscripts, which is the identical defect, and the mitigating "12288 frames of headroom" was not an invariant (it is a frame count, not a time; `reset()` rewinds the head; and `maxBlock` is the host's unclamped `samplesPerBlock`). Its payload element is now a `Sample` wrapper over one relaxed `std::atomic<float>`, exposing only relaxed `store`/`load` so the SEQ_CST `operator=` spelling is ill-formed. **The GR ring's instruction-identity claim does NOT transfer to it and must not be repeated:** four vectorised `memcpy`s become inline scalar stores (measured 4 call sites → 0, 0 lock prefixes, clang-22 and g++ 13.3), costing +0.005 % of the block period at 48 kHz/512. That ring takes NO reader-side acquire fence, because its `reset()` writes one atomic and touches no sample — there is no clear-window store for a payload load to synchronise through. **The prepared (rate, block) pair is RING metadata (ADR-0011 amended again 2026-09-02):** the GR view maps its window and scroll rate through that pair and used to read `AudioProcessor`'s plain `currentSampleRate`/`blockSize` — written by the host's `setRateAndBufferSizeDetails` on the reconfiguring thread, before `prepareToPlay`, while the tick and the paint read them: a data race. `GrHistoryBuffer` stores the pair (two relaxed, lock-free atomics) inside `clear`'s seqlock window and `prepare` owns the clear-on-change gate, so a reader bracketing a batch with the epoch reads pair and entries as one unit. The bracket's CLOSE is `batchIntact` — an acquire FENCE, then a relaxed epoch re-read — the seqlock reader Boehm (MSPC 2012) shows correct under the C++ model on weakly ordered targets, where an acquire load alone leaves the batch's earlier relaxed loads unordered against it. |
...
| Message → **Painting** (display bookkeeping the paint path reads: one editor counter; the GR history's two scroll scalars; and the spectrum's published FRAME — two 2048-bin traces plus the window and rate that make them readable) | `std::atomic` scalars, `memory_order_relaxed`, read-only from the painting side | **ADR-0027 (Accepted 2026-08-14).** `drawResizableFrame` must tell a parented pop-up from a `ResizableBorderComponent`, and asks the editor `isPopupMenuOnScreen`. That call comes from `paintOverChildren`, i.e. from the GL render thread wherever the context is attached. Permits ONE scalar carrying no payload and no ordering: it orders no other memory, is not a handshake, and a one-tick-stale read draws the border it would have drawn a frame earlier. A hook the paint path invokes is torn down only AFTER that thread is joined (`glContext.detach()` first), because assigning to a live `std::function` races on the callable regardless of what it reads. Anything with a payload, anything the paint path WRITES, or anything needing two values seen consistently is a new path again. **Widened by ADR-0038 (Accepted 2026-09-02, Architecture Review Gate cleared) to a second site carrying TWO scalars:** `GrHistoryView`'s `shownHead` and `smoothHead`, published by its frame-clock tick and read by `paintHistory` to place the 0.2.8 sub-entry scroll. They are read as a pair and are safe by VALUE rather than by consistency — every stale/fresh pairing resolves to `min (smoothHead, head + 1)`, a position between two frames the ramp itself produces, so a torn read draws a frame the display was about to draw and no vertex moves rightward (`GrHistoryView::frameFor`, pinned by `grPair`). The boundary moves rather than opening: a pair whose cross-pairings are NOT legal frames still needs consistency and is still a new path. Lock-freedom is `static_assert`ed, because a target where these are not lock-free would put a lock in the paint path. **ADR-0038 clause 7 (2026-09-02)** adds the one ordered access on this boundary: a published display estimate carries the IDENTITY of the state it describes — `publishedEpoch`, stored `release` last by the tick and loaded `acquire` first by the paint — because a phase means nothing outside the timeline it was measured in and `GrHistoryBuffer::reset()` rewinds that timeline. It orders those two publications and nothing else; relaxed would suffice on a TSO target and not on a weakly ordered one, where the epoch could become visible before the phase it announces. **ADMITTED BY ADR-0039 (Accepted 2026-09-06, Architecture Review Gate cleared):** `SpectrumView` publishes a PAYLOAD across this boundary — two 2048-bin dB traces plus the frame description that makes them readable (the audio window, and the sample rate the bins are read through) — inside a sequence bracket: counter odd (relaxed), release fence, relaxed per-bin `std::atomic<float>` stores, counter even (release); the reader acquires the counter, copies relaxed, acquire-fences, re-reads, and keeps the copy only if it is unchanged and was even, giving up after two attempts. That is exactly what the sentence three lines up excludes — "anything with a payload… or anything needing two values seen consistently is a new path again" — so it went to the gate rather than being claimed under either, and the permission is for THAT SITE and THAT MECHANISM — a single writer, a read-only painting side, a sequence bracket, and a bounded two-attempt reader whose caller stages and commits only on success. It also carries clause 7's rule one step further: the identity a payload must arrive with here is the CONFIGURATION, because a bin index means nothing without the rate that turns it into a frequency. A second payload site, a paint-path WRITE, or a second writer of the counter is a new path again and returns to this gate. |
