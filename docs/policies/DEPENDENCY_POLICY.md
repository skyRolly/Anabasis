...
7. **Re-read the code that reasons about JUCE INTERNALS, not just the code that calls JUCE.** A
   ...
   | `PluginEditor.cpp` — `popupShield` raised, and TOOLTIP/HOVER targeting | While the shield intercepts, `getComponentAt` resolves it for every pointer position, so `TooltipWindow::getTipFor` sees the shield (no tip) and the 24 Hz `hov` sweep reads `isMouseOver` GEOMETRICALLY rather than from enter/exit. Hover art survives because the synthetic move is async and `internalMouseEnter/Exit` return early for a blocked component without clearing the cached flag | Combo hover art computed from a component the pointer is logically over but which cannot receive enter/exit. A change to either the early-return or the async delivery leaves stuck or missing hover states — and the tooltip side is the half this register did not name until 2026-08-13 |
